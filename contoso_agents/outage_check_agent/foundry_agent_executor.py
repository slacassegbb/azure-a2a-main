...
class FoundryOutageCheckAgentExecutor(AgentExecutor):
    ...
    async def _get_or_create_agent(self) -> FoundryOutageCheckAgent:
        ...
        async with FoundryOutageCheckAgentExecutor._agent_lock:
            if not FoundryOutageCheckAgentExecutor._shared_foundry_agent:
                ...
                FoundryOutageCheckAgentExecutor._shared_foundry_agent = FoundryOutageCheckAgent()
    ...
    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryOutageCheckAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryOutageCheckAgentExecutor._request_semaphore:
                ...
                if current_time - FoundryOutageCheckAgentExecutor._api_call_window_start > 60:
                    FoundryOutageCheckAgentExecutor._api_call_count = 0
                    FoundryOutageCheckAgentExecutor._api_call_window_start = current_time
                
                # Check if we're approaching the API limit
                if FoundryOutageCheckAgentExecutor._api_call_count >= FoundryOutageCheckAgentExecutor._max_api_calls_per_minute:
                    wait_time = 60 - (current_time - FoundryOutageCheckAgentExecutor._api_call_window_start)
                    if wait_time > 0:
                        logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                        await asyncio.sleep(wait_time)
                        # Reset counters
                        FoundryOutageCheckAgentExecutor._api_call_count = 0
                        FoundryOutageCheckAgentExecutor._api_call_window_start = time.time()
                
                # Enforce minimum interval between requests - THIS IS THE KEY FIX
                time_since_last = current_time - FoundryOutageCheckAgentExecutor._last_request_time
                if time_since_last < FoundryOutageCheckAgentExecutor._min_request_interval:
                    sleep_time = FoundryOutageCheckAgentExecutor._min_request_interval - time_since_last
