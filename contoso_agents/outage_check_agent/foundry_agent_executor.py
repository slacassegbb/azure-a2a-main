...
from foundry_agent import FoundryOutageCheckAgent
...
class FoundryOutageCheckAgentExecutor(AgentExecutor):
    """
    ...
    _shared_foundry_agent: Optional[FoundryOutageCheckAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryOutageCheckAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    cls._shared_foundry_agent = FoundryOutageCheckAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryOutageCheckAgent:
        """Get the shared Foundry Branding agent (with fallback to lazy creation)."""
        async with FoundryOutageCheckAgentExecutor._agent_lock:
            if not FoundryOutageCheckAgentExecutor._shared_foundry_agent:
                if FoundryOutageCheckAgentExecutor._startup_complete:
                ...
                FoundryOutageCheckAgentExecutor._shared_foundry_agent = (
                    FoundryOutageCheckAgent()
                )
                await FoundryOutageCheckAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback branding agent creation completed")
            return FoundryOutageCheckAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryOutageCheckAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryOutageCheckAgentExecutor._request_semaphore:
                ...
                if (
                    current_time
                    - FoundryOutageCheckAgentExecutor._api_call_window_start
                    > 60
                ):
                    FoundryOutageCheckAgentExecutor._api_call_count = 0
                    FoundryOutageCheckAgentExecutor._api_call_window_start = (
                        current_time
                    )
                ...
                if (
                    FoundryOutageCheckAgentExecutor._api_call_count
                    >= FoundryOutageCheckAgentExecutor._max_api_calls_per_minute
                ):
                    wait_time = 60 - (
                        current_time
                        - FoundryOutageCheckAgentExecutor._api_call_window_start
                ...
                time_since_last = (
                    current_time - FoundryOutageCheckAgentExecutor._last_request_time
                )
                if (
                    time_since_last
                    < FoundryOutageCheckAgentExecutor._min_request_interval
                ):
                    sleep_time = (
                        FoundryOutageCheckAgentExecutor._min_request_interval
                        - time_since_last
                ...
                FoundryOutageCheckAgentExecutor._last_request_time = time.time()
...
def create_foundry_agent_executor(card: AgentCard) -> FoundryOutageCheckAgentExecutor:
    return FoundryOutageCheckAgentExecutor(card)
...
async def initialize_foundry_agents_at_startup():
    ...
    ```
    """
    await FoundryOutageCheckAgentExecutor.initialize_at_startup()
