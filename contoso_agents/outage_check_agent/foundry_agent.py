    ...
    async def _setup_file_search(
        ...
        async with FoundryOutageCheckAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryOutageCheckAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="outage_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryOutageCheckAgent._shared_vector_store.id}"
                )

                file_search = FileSearchTool(
                    vector_store_ids=[FoundryOutageCheckAgent._shared_vector_store.id]
                )
...
"""
        ...
        logger.info(
            f"✅ Contoso Outage Check Agent created successfully (ID: {self.agent.id})"
        )
        return self.agent
