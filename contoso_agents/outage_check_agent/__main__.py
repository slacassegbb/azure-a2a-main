...
def _resolve_default_port() -> int:
    ...
    if raw_port:
        try:
            ...
            logger.warning("Invalid A2A_PORT value '%s'; defaulting to 8102", raw_port)
    return 8102
...
def _build_agent_skills() -> list[AgentSkill]:
    """
    Define the outage check agent's skills/capabilities.
    """
    return [
        AgentSkill(
            id='outage_checking',
            name='Outage Checking',
            description="Check for local and regional internet outages. First checks for local outages at customer address, then checks regional outages if no local outage found.",
            tags=['outage', 'connectivity', 'internet', 'network'],
            examples=[
                'Check for outages at my address',
                'Is there an outage in my area?',
                'Are other customers experiencing issues?',
                'Check local and regional outages for customer CUST001'
            ],
        ),
    ]
...
def _create_agent_card(host: str, port: int) -> AgentCard:
    """
    Define the outage check agent's identity.
    """
    ...
    return AgentCard(
        ...
        description="Checks for internet service outages affecting Contoso customers. Performs local address-specific outage checks first, then regional outage checks. Provides detailed outage information including cause, estimated resolution time, and affected services.",
...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Create A2A server application for Contoso Outage Check agent."""
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse('Contoso Outage Check Agent is running!')
...
def main(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Launch A2A server mode for Contoso Outage Check Agent with startup initialization."""
    ...
    logger.info("🚀 Initializing Contoso Outage Check Agent at startup...")
    try:
    ...
    logger.info(f"Starting Contoso Outage Check Agent A2A server on {host}:{port}...")
...
def cli(host: str, port: int):
    """
    Contoso Outage Check Agent - run as an A2A server.
    
    Checks for internet service outages affecting Contoso customers.
    """
    main(host, port)
