...
from foundry_agent import FoundryModemCheckAgent
...
class FoundryModemCheckAgentExecutor(AgentExecutor):
    """
    ...
    _shared_foundry_agent: Optional[FoundryModemCheckAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryModemCheckAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    cls._shared_foundry_agent = FoundryModemCheckAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryModemCheckAgent:
        """Get the shared Foundry Branding agent (with fallback to lazy creation)."""
        async with FoundryModemCheckAgentExecutor._agent_lock:
            if not FoundryModemCheckAgentExecutor._shared_foundry_agent:
                if FoundryModemCheckAgentExecutor._startup_complete:
                ...
                FoundryModemCheckAgentExecutor._shared_foundry_agent = (
                    FoundryModemCheckAgent()
                )
                await FoundryModemCheckAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback branding agent creation completed")
            return FoundryModemCheckAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryModemCheckAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryModemCheckAgentExecutor._request_semaphore:
                # Check API call rate limiting
                current_time = time.time()

                # Reset the window if it's been more than a minute
                if (
                    current_time - FoundryModemCheckAgentExecutor._api_call_window_start
                    > 60
                ):
                    FoundryModemCheckAgentExecutor._api_call_count = 0
                    FoundryModemCheckAgentExecutor._api_call_window_start = current_time

                # Check if we're approaching the API limit
                if (
                    FoundryModemCheckAgentExecutor._api_call_count
                    >= FoundryModemCheckAgentExecutor._max_api_calls_per_minute
                ):
                    wait_time = 60 - (
                        current_time
                        - FoundryModemCheckAgentExecutor._api_call_window_start
                    )
                    if wait_time > 0:
                        logger.warning(
                ...
                time_since_last = (
                    current_time - FoundryModemCheckAgentExecutor._last_request_time
                )
                if (
                    time_since_last
                    < FoundryModemCheckAgentExecutor._min_request_interval
                ):
                    sleep_time = (
                        FoundryModemCheckAgentExecutor._min_request_interval
                        - time_since_last
                ...
                FoundryModemCheckAgentExecutor._last_request_time = time.time()
...
def create_foundry_agent_executor(card: AgentCard) -> FoundryModemCheckAgentExecutor:
    return FoundryModemCheckAgentExecutor(card)
...
async def initialize_foundry_agents_at_startup():
    ...
    ```
    """
    await FoundryModemCheckAgentExecutor.initialize_at_startup()
