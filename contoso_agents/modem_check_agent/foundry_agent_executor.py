...
class FoundryModemCheckAgentExecutor(AgentExecutor):
    ...
    async def _get_or_create_agent(self) -> FoundryModemCheckAgent:
        ...
        async with FoundryModemCheckAgentExecutor._agent_lock:
            if not FoundryModemCheckAgentExecutor._shared_foundry_agent:
                ...
                FoundryModemCheckAgentExecutor._shared_foundry_agent = FoundryModemCheckAgent()
    ...
    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryModemCheckAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryModemCheckAgentExecutor._request_semaphore:
                ...
                if current_time - FoundryModemCheckAgentExecutor._api_call_window_start > 60:
                ...
                if FoundryModemCheckAgentExecutor._api_call_count >= FoundryModemCheckAgentExecutor._max_api_calls_per_minute:
                    wait_time = 60 - (current_time - FoundryModemCheckAgentExecutor._api_call_window_start)
                    if wait_time > 0:
                        logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                        await asyncio.sleep(wait_time)
                        # Reset counters
                        FoundryModemCheckAgentExecutor._api_call_count = 0
                        FoundryModemCheckAgentExecutor._api_call_window_start = time.time()
                
                # Enforce minimum interval between requests - THIS IS THE KEY FIX
                time_since_last = current_time - FoundryModemCheckAgentExecutor._last_request_time
                if time_since_last < FoundryModemCheckAgentExecutor._min_request_interval:
                    sleep_time = FoundryModemCheckAgentExecutor._min_request_interval - time_since_last
