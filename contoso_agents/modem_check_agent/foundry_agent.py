    ...
    async def _setup_file_search(
        ...
        async with FoundryModemCheckAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryModemCheckAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="modem_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryModemCheckAgent._shared_vector_store.id}"
                )

                file_search = FileSearchTool(
                    vector_store_ids=[FoundryModemCheckAgent._shared_vector_store.id]
                )
...
"""
        ...
        logger.info(
            f"✅ Contoso Modem Check Agent created successfully (ID: {self.agent.id})"
        )
        return self.agent
