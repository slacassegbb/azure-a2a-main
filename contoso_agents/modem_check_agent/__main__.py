...
def _resolve_default_port() -> int:
    ...
    if raw_port:
        try:
            ...
            logger.warning("Invalid A2A_PORT value '%s'; defaulting to 8103", raw_port)
    return 8103
...
def _build_agent_skills() -> list[AgentSkill]:
    """
    Define the modem check agent's skills/capabilities.
    """
    return [
        AgentSkill(
            ...
            examples=[
                ...
                "My modem light is yellow",
                "Check backend modem configuration",
                "Modem showing red light",
            ],
        ),
    ]
...
def _create_agent_card(host: str, port: int) -> AgentCard:
    """
    Define the modem check agent's identity.
    """
...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Create A2A server application for Contoso Modem Check agent."""
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse("Contoso Modem Check Agent is running!")
...
def main(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Launch A2A server mode for Contoso Modem Check Agent with startup initialization."""
    ...
    logger.info("🚀 Initializing Contoso Modem Check Agent at startup...")
    try:
    ...
    logger.info(f"Starting Contoso Modem Check Agent A2A server on {host}:{port}...")
...
def cli(host: str, port: int):
    """
    Contoso Modem Check Agent - run as an A2A server.

    Performs comprehensive modem diagnostics including LED analysis.
    """
    main(host, port)
