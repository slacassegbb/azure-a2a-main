            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryAuthenticationAgent._shared_uploaded_files.append(file.id)
                ...
                FoundryAuthenticationAgent._shared_vector_store = project_client.agents.vector_stores.create_and_poll(
                    file_ids=file_ids, 
                    name="authentication_vectorstore"
                )
                logger.info(f"Created shared vector store: {FoundryAuthenticationAgent._shared_vector_store.id}")
                
                file_search = FileSearchTool(vector_store_ids=[FoundryAuthenticationAgent._shared_vector_store.id])
...
"""
        ...
        logger.info(f"✅ Contoso Authentication Agent created successfully (ID: {self.agent.id})")
        return self.agent
    ...
    async def run_conversation_stream(self, thread_id: str, user_message: str):
        ...
        logger.info(f"🟢 [AUTH AGENT] run_conversation_stream called for thread {thread_id}")
        project_client = self._get_project_client()
        agents_client = project_client.agents
        ...
        logger.info(f"🟢 [AUTH AGENT] Creating and processing run for thread {thread_id}")
