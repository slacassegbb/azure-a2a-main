...
def _resolve_default_port() -> int:
    ...
    if raw_port:
        try:
            ...
            logger.warning("Invalid A2A_PORT value '%s'; defaulting to 8101", raw_port)
    return 8101
...
def _build_agent_skills() -> List[AgentSkill]:
    """
    Define the authentication agent's skills/capabilities.
    """
    return [
        AgentSkill(
...
def _create_agent_card(host: str, port: int) -> AgentCard:
    """
    Define the authentication agent's identity.
    """
...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Create A2A server application for Contoso Authentication agent."""
...
def main(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    logger.info("🚀 Initializing Contoso Authentication Agent at startup...")
    try:
