...
class FoundryAuthenticationAgentExecutor(AgentExecutor):
    ...
    async def _get_or_create_agent(self) -> FoundryAuthenticationAgent:
        ...
        async with FoundryAuthenticationAgentExecutor._agent_lock:
            if not FoundryAuthenticationAgentExecutor._shared_foundry_agent:
                ...
                FoundryAuthenticationAgentExecutor._shared_foundry_agent = FoundryAuthenticationAgent()
    ...
    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryAuthenticationAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryAuthenticationAgentExecutor._request_semaphore:
                ...
                if current_time - FoundryAuthenticationAgentExecutor._api_call_window_start > 60:
                    FoundryAuthenticationAgentExecutor._api_call_count = 0
                    FoundryAuthenticationAgentExecutor._api_call_window_start = current_time
                
                # Check if we're approaching the API limit
                if FoundryAuthenticationAgentExecutor._api_call_count >= FoundryAuthenticationAgentExecutor._max_api_calls_per_minute:
                    wait_time = 60 - (current_time - FoundryAuthenticationAgentExecutor._api_call_window_start)
                    if wait_time > 0:
                        logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                        await asyncio.sleep(wait_time)
                        # Reset counters
                        FoundryAuthenticationAgentExecutor._api_call_count = 0
                        FoundryAuthenticationAgentExecutor._api_call_window_start = time.time()
                
                # Enforce minimum interval between requests - THIS IS THE KEY FIX
                time_since_last = current_time - FoundryAuthenticationAgentExecutor._last_request_time
                if time_since_last < FoundryAuthenticationAgentExecutor._min_request_interval:
                    sleep_time = FoundryAuthenticationAgentExecutor._min_request_interval - time_since_last
...
def create_foundry_agent_executor(card: AgentCard) -> FoundryAuthenticationAgentExecutor:
