...
class FoundryAuthenticationAgentExecutor(AgentExecutor):
    """
    ...
    _shared_foundry_agent: Optional[FoundryAuthenticationAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryAuthenticationAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    cls._shared_foundry_agent = FoundryAuthenticationAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryAuthenticationAgent:
        """Get the shared Foundry Branding agent (with fallback to lazy creation)."""
        async with FoundryAuthenticationAgentExecutor._agent_lock:
            if not FoundryAuthenticationAgentExecutor._shared_foundry_agent:
                if FoundryAuthenticationAgentExecutor._startup_complete:
                ...
                FoundryAuthenticationAgentExecutor._shared_foundry_agent = (
                    FoundryAuthenticationAgent()
                )
                await FoundryAuthenticationAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback branding agent creation completed")
            return FoundryAuthenticationAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryAuthenticationAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryAuthenticationAgentExecutor._request_semaphore:
                ...
                if (
                    current_time
                    - FoundryAuthenticationAgentExecutor._api_call_window_start
                    > 60
                ):
                    FoundryAuthenticationAgentExecutor._api_call_count = 0
                    FoundryAuthenticationAgentExecutor._api_call_window_start = (
                        current_time
                    )
                ...
                if (
                    FoundryAuthenticationAgentExecutor._api_call_count
                    >= FoundryAuthenticationAgentExecutor._max_api_calls_per_minute
                ):
                    wait_time = 60 - (
                        current_time
                        - FoundryAuthenticationAgentExecutor._api_call_window_start
                ...
                time_since_last = (
                    current_time - FoundryAuthenticationAgentExecutor._last_request_time
                )
                if (
                    time_since_last
                    < FoundryAuthenticationAgentExecutor._min_request_interval
                ):
                    sleep_time = (
                        FoundryAuthenticationAgentExecutor._min_request_interval
                        - time_since_last
                ...
                FoundryAuthenticationAgentExecutor._last_request_time = time.time()
...
def create_foundry_agent_executor(
    card: AgentCard,
) -> FoundryAuthenticationAgentExecutor:
    return FoundryAuthenticationAgentExecutor(card)
...
async def initialize_foundry_agents_at_startup():
    ...
    ```
    """
    await FoundryAuthenticationAgentExecutor.initialize_at_startup()
