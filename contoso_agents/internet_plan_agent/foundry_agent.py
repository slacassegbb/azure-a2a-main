    ...
    async def _setup_file_search(
        ...
        async with FoundryInternetPlanAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryInternetPlanAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="plan_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryInternetPlanAgent._shared_vector_store.id}"
                )

                file_search = FileSearchTool(
                    vector_store_ids=[FoundryInternetPlanAgent._shared_vector_store.id]
                )
...
"""
        ...
        logger.info(
            f"✅ Contoso Internet Plan Agent created successfully (ID: {self.agent.id})"
        )
        return self.agent
