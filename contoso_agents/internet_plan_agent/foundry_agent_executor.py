...
from foundry_agent import FoundryInternetPlanAgent
...
class FoundryInternetPlanAgentExecutor(AgentExecutor):
    """
    ...
    _shared_foundry_agent: Optional[FoundryInternetPlanAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryInternetPlanAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    cls._shared_foundry_agent = FoundryInternetPlanAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryInternetPlanAgent:
        """Get the shared Foundry Branding agent (with fallback to lazy creation)."""
        async with FoundryInternetPlanAgentExecutor._agent_lock:
            if not FoundryInternetPlanAgentExecutor._shared_foundry_agent:
                if FoundryInternetPlanAgentExecutor._startup_complete:
                ...
                FoundryInternetPlanAgentExecutor._shared_foundry_agent = (
                    FoundryInternetPlanAgent()
                )
                await FoundryInternetPlanAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback branding agent creation completed")
            return FoundryInternetPlanAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryInternetPlanAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryInternetPlanAgentExecutor._request_semaphore:
                ...
                if (
                    current_time
                    - FoundryInternetPlanAgentExecutor._api_call_window_start
                    > 60
                ):
                    FoundryInternetPlanAgentExecutor._api_call_count = 0
                    FoundryInternetPlanAgentExecutor._api_call_window_start = (
                        current_time
                    )
                ...
                if (
                    FoundryInternetPlanAgentExecutor._api_call_count
                    >= FoundryInternetPlanAgentExecutor._max_api_calls_per_minute
                ):
                    wait_time = 60 - (
                        current_time
                        - FoundryInternetPlanAgentExecutor._api_call_window_start
                ...
                time_since_last = (
                    current_time - FoundryInternetPlanAgentExecutor._last_request_time
                )
                if (
                    time_since_last
                    < FoundryInternetPlanAgentExecutor._min_request_interval
                ):
                    sleep_time = (
                        FoundryInternetPlanAgentExecutor._min_request_interval
                        - time_since_last
                ...
                FoundryInternetPlanAgentExecutor._last_request_time = time.time()
...
def create_foundry_agent_executor(card: AgentCard) -> FoundryInternetPlanAgentExecutor:
    return FoundryInternetPlanAgentExecutor(card)
...
async def initialize_foundry_agents_at_startup():
    ...
    ```
    """
    await FoundryInternetPlanAgentExecutor.initialize_at_startup()
