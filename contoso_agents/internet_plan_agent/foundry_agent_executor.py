...
class FoundryInternetPlanAgentExecutor(AgentExecutor):
    ...
    async def _get_or_create_agent(self) -> FoundryInternetPlanAgent:
        ...
        async with FoundryInternetPlanAgentExecutor._agent_lock:
            if not FoundryInternetPlanAgentExecutor._shared_foundry_agent:
                ...
                FoundryInternetPlanAgentExecutor._shared_foundry_agent = FoundryInternetPlanAgent()
    ...
    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryInternetPlanAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryInternetPlanAgentExecutor._request_semaphore:
                ...
                if current_time - FoundryInternetPlanAgentExecutor._api_call_window_start > 60:
                    FoundryInternetPlanAgentExecutor._api_call_count = 0
                    FoundryInternetPlanAgentExecutor._api_call_window_start = current_time
                
                # Check if we're approaching the API limit
                if FoundryInternetPlanAgentExecutor._api_call_count >= FoundryInternetPlanAgentExecutor._max_api_calls_per_minute:
                    wait_time = 60 - (current_time - FoundryInternetPlanAgentExecutor._api_call_window_start)
                    if wait_time > 0:
                        logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                        await asyncio.sleep(wait_time)
                        # Reset counters
                        FoundryInternetPlanAgentExecutor._api_call_count = 0
                        FoundryInternetPlanAgentExecutor._api_call_window_start = time.time()
                
                # Enforce minimum interval between requests - THIS IS THE KEY FIX
                time_since_last = current_time - FoundryInternetPlanAgentExecutor._last_request_time
                if time_since_last < FoundryInternetPlanAgentExecutor._min_request_interval:
                    sleep_time = FoundryInternetPlanAgentExecutor._min_request_interval - time_since_last
