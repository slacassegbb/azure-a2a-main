...
def _resolve_default_port() -> int:
    ...
    if raw_port:
        try:
            ...
            logger.warning("Invalid A2A_PORT value '%s'; defaulting to 8104", raw_port)
    return 8104
...
def _build_agent_skills() -> list[AgentSkill]:
    """
    Define the internet plan agent's skills/capabilities.
    """
    return [
        AgentSkill(
            id="plan_checking",
            name="Internet Plan & Billing Check",
            description="Review customer internet plans, data usage limits, billing status, and payment history. Identifies customers who exceeded data caps or have unpaid bills that may affect service access.",
            tags=["plan", "billing", "data-usage", "payment", "subscription"],
            examples=[
                "Check my internet plan details",
                "How much data have I used?",
                "Have I exceeded my data limit?",
                "Check if my bills are paid",
                "Verify customer plan and billing status",
            ],
        ),
    ]
...
def _create_agent_card(host: str, port: int) -> AgentCard:
    """
    Define the internet plan agent's identity.
    """
    ...
    return AgentCard(
        ...
        description="Manages internet plan verification, data usage monitoring, and billing status checks. Reviews customer plans (unlimited or capped), calculates data overages and charges, checks payment history for last 3 bills, and determines if customer should have active internet access. Alerts when data limits are exceeded or bills are unpaid.",
...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Create A2A server application for Contoso Internet Plan agent."""
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse("Contoso Internet Plan Agent is running!")
...
def main(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    """Launch A2A server mode for Contoso Internet Plan Agent with startup initialization."""
    ...
    logger.info("🚀 Initializing Contoso Internet Plan Agent at startup...")
    try:
    ...
    logger.info(f"Starting Contoso Internet Plan Agent A2A server on {host}:{port}...")
...
def cli(host: str, port: int):
    """
    Contoso Internet Plan Agent - run as an A2A server.

    Manages internet plan verification, data usage, and billing checks.
    """
    main(host, port)
