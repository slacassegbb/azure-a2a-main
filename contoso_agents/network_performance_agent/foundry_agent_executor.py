...
class FoundryNetworkPerformanceAgentExecutor(AgentExecutor):
    ...
    async def _get_or_create_agent(self) -> FoundryNetworkPerformanceAgent:
        ...
        async with FoundryNetworkPerformanceAgentExecutor._agent_lock:
            if not FoundryNetworkPerformanceAgentExecutor._shared_foundry_agent:
                ...
                FoundryNetworkPerformanceAgentExecutor._shared_foundry_agent = FoundryNetworkPerformanceAgent()
    ...
    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryNetworkPerformanceAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryNetworkPerformanceAgentExecutor._request_semaphore:
                ...
                if current_time - FoundryNetworkPerformanceAgentExecutor._api_call_window_start > 60:
                    FoundryNetworkPerformanceAgentExecutor._api_call_count = 0
                    FoundryNetworkPerformanceAgentExecutor._api_call_window_start = current_time
                
                # Check if we're approaching the API limit
                if FoundryNetworkPerformanceAgentExecutor._api_call_count >= FoundryNetworkPerformanceAgentExecutor._max_api_calls_per_minute:
                    wait_time = 60 - (current_time - FoundryNetworkPerformanceAgentExecutor._api_call_window_start)
                    if wait_time > 0:
                        logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                        await asyncio.sleep(wait_time)
                        # Reset counters
                        FoundryNetworkPerformanceAgentExecutor._api_call_count = 0
                        FoundryNetworkPerformanceAgentExecutor._api_call_window_start = time.time()
                
                # Enforce minimum interval between requests - THIS IS THE KEY FIX
                time_since_last = current_time - FoundryNetworkPerformanceAgentExecutor._last_request_time
                if time_since_last < FoundryNetworkPerformanceAgentExecutor._min_request_interval:
                    sleep_time = FoundryNetworkPerformanceAgentExecutor._min_request_interval - time_since_last
...
def create_foundry_agent_executor(card: AgentCard) -> FoundryNetworkPerformanceAgentExecutor:
