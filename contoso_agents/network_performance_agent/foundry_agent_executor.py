...
from .foundry_agent import FoundryNetworkPerformanceAgent
...
class FoundryNetworkPerformanceAgentExecutor(AgentExecutor):
    """
    ...
    _shared_foundry_agent: Optional[FoundryNetworkPerformanceAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryNetworkPerformanceAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    cls._shared_foundry_agent = FoundryNetworkPerformanceAgent()
                    await cls._shared_foundry_agent.create_agent()
                    cls._startup_complete = True
                    logger.info(
                        "✅ Foundry agent startup initialization completed successfully"
                    )
    ...
    async def _get_or_create_agent(self) -> FoundryNetworkPerformanceAgent:
        """Get the shared Foundry Branding agent (with fallback to lazy creation)."""
        async with FoundryNetworkPerformanceAgentExecutor._agent_lock:
            if not FoundryNetworkPerformanceAgentExecutor._shared_foundry_agent:
                if FoundryNetworkPerformanceAgentExecutor._startup_complete:
                ...
                FoundryNetworkPerformanceAgentExecutor._shared_foundry_agent = (
                    FoundryNetworkPerformanceAgent()
                )
                await FoundryNetworkPerformanceAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback branding agent creation completed")
            return FoundryNetworkPerformanceAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryNetworkPerformanceAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def _process_request(
        ...
        logger.info(
            f"🟡 [AUTH EXECUTOR] _process_request called with {len(message_parts)} parts"
        )
        ...
        try:
            ...
            logger.error(
                f"❌ [AUTH EXECUTOR] Exception in _process_request: {e}", exc_info=True
            )
            await task_updater.failed(
    ...
    async def execute(
        ...
        logger.info(
            f"🔵 [AUTH EXECUTOR] Starting execute for context {context.context_id}"
        )
        logger.info(
            f"🔵 [AUTH EXECUTOR] Request has message: {context.message is not None}"
        )
        if context.message:
            logger.info(
                f"🔵 [AUTH EXECUTOR] Message has {len(context.message.parts)} parts"
            )

        try:
            # CRITICAL: Apply rate limiting at the execute level to control between different user requests
            async with FoundryNetworkPerformanceAgentExecutor._request_semaphore:
                ...
                if (
                    current_time
                    - FoundryNetworkPerformanceAgentExecutor._api_call_window_start
                    > 60
                ):
                    FoundryNetworkPerformanceAgentExecutor._api_call_count = 0
                    FoundryNetworkPerformanceAgentExecutor._api_call_window_start = (
                        current_time
                    )
                ...
                if (
                    FoundryNetworkPerformanceAgentExecutor._api_call_count
                    >= FoundryNetworkPerformanceAgentExecutor._max_api_calls_per_minute
                ):
                    wait_time = 60 - (
                        current_time
                        - FoundryNetworkPerformanceAgentExecutor._api_call_window_start
                ...
                time_since_last = (
                    current_time
                    - FoundryNetworkPerformanceAgentExecutor._last_request_time
                )
                if (
                    time_since_last
                    < FoundryNetworkPerformanceAgentExecutor._min_request_interval
                ):
                    sleep_time = (
                        FoundryNetworkPerformanceAgentExecutor._min_request_interval
                        - time_since_last
                ...
                FoundryNetworkPerformanceAgentExecutor._last_request_time = time.time()
                ...
                logger.info(
                    f"🔵 [AUTH EXECUTOR] Starting work on task {context.task_id}"
                )
                await updater.start_work()

                logger.info(
                    f"🔵 [AUTH EXECUTOR] Processing request with {len(context.message.parts if context.message else [])} parts"
                )
                ...
                logger.info(
                    f"✅ [AUTH EXECUTOR] Completed execution for {context.context_id}"
                )
        except Exception as e:
...
def create_foundry_agent_executor(
    card: AgentCard,
) -> FoundryNetworkPerformanceAgentExecutor:
    return FoundryNetworkPerformanceAgentExecutor(card)
...
async def initialize_foundry_agents_at_startup():
    ...
    ```
    """
    await FoundryNetworkPerformanceAgentExecutor.initialize_at_startup()
