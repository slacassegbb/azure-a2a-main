...
class FoundryNetworkPerformanceAgent:
    ...
    async def _setup_file_search(
        self,
        files_directory: str = "documents",
        vector_store_name: str = "network_performance_vectorstore",
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryNetworkPerformanceAgent._shared_uploaded_files.append(
                            file.id
                        )
                        logger.info(
                ...
                FoundryNetworkPerformanceAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name=vector_store_name
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryNetworkPerformanceAgent._shared_vector_store.id}"
                )

                file_search = FileSearchTool(
                    vector_store_ids=[
                        FoundryNetworkPerformanceAgent._shared_vector_store.id
                    ]
                )
    ...
    async def create_agent(self) -> Agent:
        ...
        file_search_tool = await self._setup_file_search(
            documents_directory, vector_store_name
        )
        if file_search_tool:
        ...
        logger.info(
            f"✅ Contoso Network Performance Agent created successfully (ID: {self.agent.id})"
        )
        return self.agent
    ...
    async def _handle_tool_calls(self, run: ThreadRun, thread_id: str):
        ...
        try:
            ...
            async def handle_single_tool_call(tool_call):
                ...
                if function_name == "retrieve_operational_context":
                    ...
                    try:
                        ...
                        logger.info(
                            f"📊 Executing retrieve_operational_context with query: {query}"
                        )
                        ...
                        logger.info(
                            f"✅ Fabric tool returned {len(result)} characters of data"
                        )

                        return {"tool_call_id": tool_call.id, "output": result}
                    except Exception as e:
                        logger.error(
                            f"❌ Error executing retrieve_operational_context: {e}"
                        )
                        ...
                        return {"tool_call_id": tool_call.id, "output": error_output}
                else:
            ...
            if not tool_outputs:
                logger.info(
                    "No valid tool outputs generated - submitting empty outputs"
                )
                tool_outputs = [
    ...
    async def run_conversation_stream(self, thread_id: str, user_message: str):
        ...
        logger.info(
            f"🟢 [NETWORK AGENT] run_conversation_stream called for thread {thread_id}"
        )
        project_client = self._get_project_client()
        agents_client = project_client.agents
        ...
        run = agents_client.runs.create(thread_id=thread_id, agent_id=self.agent.id)
        ...
        while iteration < max_iterations:
            ...
            if run.status == "requires_action":
                ...
                logger.info(
                    f"🟢 [NETWORK AGENT] Run reached terminal state: {run.status}"
                )
                break
            else:
