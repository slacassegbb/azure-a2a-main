            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryTechnicalDispatchAgent._shared_uploaded_files.append(
                            file.id
                        )
                        logger.info(
                ...
                FoundryTechnicalDispatchAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="technical_dispatch_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryTechnicalDispatchAgent._shared_vector_store.id}"
                )

                file_search = FileSearchTool(
                    vector_store_ids=[
                        FoundryTechnicalDispatchAgent._shared_vector_store.id
                    ]
                )
...
Remember: You are the final decision point in the troubleshooting workflow. Make clear, actionable decisions based on all available diagnostic data."""
        ...
        logger.info(
            f"✅ Contoso Technical Dispatch Agent created successfully (ID: {self.agent.id})"
        )
        return self.agent
    ...
    async def run_conversation_stream(self, thread_id: str, user_message: str):
        ...
        logger.info(
            f"🟢 [DISPATCH AGENT] run_conversation_stream called for thread {thread_id}"
        )
        project_client = self._get_project_client()
        agents_client = project_client.agents
        ...
        logger.info(
            f"🟢 [DISPATCH AGENT] Creating and processing run for thread {thread_id}"
        )
