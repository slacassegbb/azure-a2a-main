...
class FoundryTechnicalDispatchAgentExecutor(AgentExecutor):
    ...
    @classmethod
    async def _send_human_response_internal(
        cls, context_id: str, response: str
    ) -> bool:
        """
    ...
    async def send_human_response(self, context_id: str, response: str) -> bool:
        ...
        success = await self.__class__._send_human_response_internal(
            context_id, response
        )
        ...
        async with FoundryTechnicalDispatchAgentExecutor._agent_lock:
            if not FoundryTechnicalDispatchAgentExecutor._shared_foundry_agent:
                if FoundryTechnicalDispatchAgentExecutor._startup_complete:
                    ...
                    raise RuntimeError(
                        "Technical Dispatch agent startup initialization failed - agent not available"
                    )

                # Fallback to lazy creation if startup wasn't called
                logger.warning(
                    "⚠️ Technical Dispatch agent not initialized at startup, falling back to lazy creation..."
                )
                FoundryTechnicalDispatchAgentExecutor._shared_foundry_agent = (
                    FoundryTechnicalDispatchAgent()
                )
    ...
    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryTechnicalDispatchAgent] = None
    ) -> str:
        if agent is None:
            ...
            if responses:
                ...
                if "HUMAN_ESCALATION_REQUIRED" in final_response:
                    ...
                    async with FoundryTechnicalDispatchAgentExecutor._escalation_lock:
                        FoundryTechnicalDispatchAgentExecutor._pending_escalations[
                            context_id
                        ] = final_response
                    ...
                    logger.info(
                        f"⏳ Waiting for human response for context {context_id}..."
                    )
                    ...
                    human_response = (
                        FoundryTechnicalDispatchAgentExecutor._escalation_responses.get(
                            context_id
                        )
                    )
                    if human_response:
                        logger.info(
                            f"✅ Received human response for context {context_id}"
                        )
                        # Send human response back through agent and get final response
                        final_responses = []
                        async for event in agent.run_conversation_stream(
                            thread_id, f"[HUMAN EXPERT RESPONSE]: {human_response}"
                        ):
                        ...
                        logger.info(
                            f"✅ Final response after human input: {final_response[:200]}..."
                        )
                    ...
                    async with FoundryTechnicalDispatchAgentExecutor._escalation_lock:
                        FoundryTechnicalDispatchAgentExecutor._pending_escalations.pop(
                            context_id, None
                        )
                        FoundryTechnicalDispatchAgentExecutor._escalation_responses.pop(
                            context_id, None
                        )
    ...
    async def execute(
        ...
        try:
            ...
            async with FoundryTechnicalDispatchAgentExecutor._request_semaphore:
                ...
                if (
                    current_time
                    - FoundryTechnicalDispatchAgentExecutor._api_call_window_start
                    > 60
                ):
                ...
                if (
                    FoundryTechnicalDispatchAgentExecutor._api_call_count
                    >= FoundryTechnicalDispatchAgentExecutor._max_api_calls_per_minute
                ):
                    wait_time = 60 - (
                        current_time
                        - FoundryTechnicalDispatchAgentExecutor._api_call_window_start
                ...
                time_since_last = (
                    current_time
                    - FoundryTechnicalDispatchAgentExecutor._last_request_time
                )
                if (
                    time_since_last
                    < FoundryTechnicalDispatchAgentExecutor._min_request_interval
                ):
                    sleep_time = (
                        FoundryTechnicalDispatchAgentExecutor._min_request_interval
                        - time_since_last
...
def create_foundry_agent_executor(
    card: AgentCard,
) -> FoundryTechnicalDispatchAgentExecutor:
