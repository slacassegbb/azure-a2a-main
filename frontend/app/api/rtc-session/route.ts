...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) throw new Error("AZURE_VOICE_API_KEY not configured");
    ...
    const tokenRes = await fetch(`${baseUrl}/client_secrets`, {
      ...
      body: JSON.stringify({ session: { type: "realtime", model: deployment } }),
    });

    if (!tokenRes.ok) {
      const detail = await tokenRes.text();
      return NextResponse.json({ error: `Token request failed: ${tokenRes.status}`, detail }, { status: tokenRes.status });
    }
    ...
    if (!sdpRes.ok) {
      ...
      return NextResponse.json({ error: `SDP exchange failed: ${sdpRes.status}`, detail }, { status: sdpRes.status });
    }
