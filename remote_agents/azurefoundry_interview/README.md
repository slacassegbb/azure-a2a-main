...
**An intelligent interview agent that conducts consultative discovery conversations to qualify leads for AI consulting services.**

This agent specializes in conducting thoughtful, probing interviews to understand customer needs, gather comprehensive information, and qualify leads. It uses the A2A protocol's `input_required` feature to ask follow-up questions and gather detailed information before completing the conversation.
...
- ✅ **Input Required Support** – Uses A2A's `input_required` to probe for more information
- ✅ **Interview Configuration** – Easy-to-edit `interview_config.py` for questions and behavior
- ✅ **File Search Integration** – Grounds responses in your company documents
- ✅ **Intelligent Probing** – Automatically detects when to ask follow-up questions
- ✅ **Lead Qualification** – Systematically gathers information across key areas
- ✅ **Gradio UI** – Built-in chat interface for testing interviews
...
This interview agent is designed to:

1. **Conduct Consultative Interviews** – Engage customers in natural, flowing conversations
2. **Probe Intelligently** – Ask follow-up questions based on responses
3. **Gather Comprehensive Information** – Systematically collect details about:
   - Company information and industry context
   - Business challenges and AI needs
   - Technical environment and constraints
   - Timeline and budget expectations
   - Stakeholders and decision processes
   - Contact information for follow-up
4. **Use Input Required** – Leverage A2A's `input_required` state to ensure information is gathered before proceeding
5. **Qualify Leads** – Assess fit and recommend appropriate next steps

## 🎨 Easy Customization

**All customization is centralized in `interview_config.py`** - no need to edit complex code!

You can easily customize:
- Company name and description
- Interview questions for each category
- Agent personality and conversation style
- Qualification criteria
- Greeting and closing messages
- Follow-up question templates

Simply edit `interview_config.py` and restart the agent. No coding required!
...
1. **Edit `interview_config.py`** with your company information:
   ```python
   # Update company details
   COMPANY_NAME = "Your AI Consulting Company"
   COMPANY_DESCRIPTION = "An AI consulting company specializing in..."
...
2. **Customize `documents/INTERVIEW_GUIDE.md`** with your:
   - Actual services and offerings
   - Real case studies and success stories
   - Pricing models and engagement types
   - Qualification criteria
   - Common questions and answers

3. **Add additional documents** to the `documents/` folder:
...
The agent will automatically index these documents and use them to ground its responses via Azure AI Foundry's file search capability.

---

### Step 3: (Optional) Advanced Customization

**Note:** For most use cases, editing `interview_config.py` and the documents is sufficient. These advanced options are available if needed:

#### 3.1 Modify Agent Instructions (`foundry_agent.py`)

The agent instructions are automatically built from `interview_config.py`. If you need to make structural changes to how the agent thinks:
