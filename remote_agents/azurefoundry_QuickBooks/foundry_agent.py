...
class FoundryQuickBooksAgent:
    ...
    async def create_agent(self) -> Agent:
        ...
        # Start with MCP tools using the new McpTool class
        logger.info("🔍 CREATING MCP TOOL CONNECTION...")
        logger.info(f"   Server URL: {QUICKBOOKS_MCP_URL}")
        ...
        try:
            ...
            async def test_mcp_basic():
                try:
                    ...
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        async with client.stream("GET", QUICKBOOKS_MCP_URL, headers=headers) as response:
            ...
            logger.info(f"🔧 Using MCP server URL: {QUICKBOOKS_MCP_URL}")
            mcp_tool = McpTool(
