...
class ServiceNowClient:
    ...
    async def _get_session(self):
        ...
        if self.session is None:
            ...
            timeout = aiohttp.ClientTimeout(
                total=60
            )  # bump timeout for slower instances
    ...
    async def search_records(
        self, table: str = "incident", query: str = "", limit: int = 10
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            generic_intent = (
                ...
                or (
                    ...
                    and any(kw in normalized for kw in ["all", "list", "show", "find"])
                )
            )
            if generic_intent:
                ...
                if is_encoded:
                    ...
                    def _normalize_like(match: re.Match) -> str:
                        ...
                        needs_encoding = cleaned == decoded and any(
                            ch in decoded for ch in [" ", "\t"]
                        )
                        encoded = (
                            urllib.parse.quote(decoded) if needs_encoding else decoded
                        )
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.get(url) as response:
                    ...
                    if response.status == 200:
                        ...
                        if (
                            (not result.get("records"))
                            and (not normalized or not is_encoded)
                            and not fallback_applied
                        ):
                        ...
                        if (not records) and is_encoded and "^" in effective_query:
                            segments = [
                                seg for seg in effective_query.split("^") if seg
                            ]
                            ...
                            if filtered_segments and len(filtered_segments) != len(
                                segments
                            ):
                                ...
                                print(
                                    f"🔍 Fallback API call (reduced filters): {reduced_url}"
                                )
                                async with session.get(reduced_url) as reduced_resp:
                                    print(
                                        f"🔍 Fallback (reduced) status: {reduced_resp.status}"
                                    )
                            ...
                            if not fallback_applied:
                                caller_like_segments = [
                                    seg
                                    for seg in segments
                                    if "caller_idlike" in seg.lower()
                                ]
                                if caller_like_segments:
                                    import urllib.parse

                                    target_value = (
                                        caller_like_segments[0]
                                        .split("LIKE", 1)[1]
                                        .strip()
                                        .strip('"')
                                        .strip("'")
                                    ...
                                    if target_value:
                                        ...
                                        caller_fallback_url = f"{self.instance_url}/api/now/table/{table}?sysparm_limit={limit}&sysparm_query={caller_fallback_query}"
                                        print(
                                            f"🔍 Fallback API call (caller content search): {caller_fallback_url}"
                                        )
                                        async with session.get(
                                            caller_fallback_url
                                        ) as caller_resp:
                                            print(
                                                f"🔍 Fallback (caller search) status: {caller_resp.status}"
                                            )
                                            if caller_resp.status == 200:
                                                caller_data = await caller_resp.json()
                                                caller_records = caller_data.get(
                                                    "result", []
                                                )
                                                if caller_records:
                                                    result = {
                        ...
                        print(
                            f"🔍 ServiceNow API error: HTTP {response.status}: {error_text}"
                        )
                        return {
                            "success": False,
            ...
            return {"success": False, "error": str(e), "table": table, "query": query}

    async def create_incident(
        self, short_description: str, description: str = None, priority: int = 3
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "incident_data": incident_data}
...
class ServiceNowMCP:
    ...
    def __init__(
        self, instance_url: str, auth: ServiceNowAuth, name: str = "ServiceNow MCP"
    ):
        ...
        print(
            f"DEBUG: Password from os.environ before set: {os.getenv('SERVICENOW_PASSWORD')}"
        )
        ...
        if instance_url and "dev" in instance_url:
            ...
            if instance_match:
                ...
                os.environ[
                    "SERVICENOW_INSTANCE"
                ] = "dev355156"  # fallback to correct instance
        else:
            os.environ[
                "SERVICENOW_INSTANCE"
            ] = "dev355156"  # fallback to correct instance
        ...
        print(
            f"DEBUG: Password from os.environ after set: {os.getenv('SERVICENOW_PASSWORD')}"
        )
        ...
        logger.info(
            f"🔧 MCP Server attributes: {[attr for attr in dir(original_mcp_server) if not attr.startswith('_')]}"
        )
        ...
        if hasattr(self.mcp, "_mcp_list_tools"):
            ...
            async def debug_mcp_list_tools(request):
                ...
                try:
                    ...
                    if hasattr(result, "tools") and result.tools:
                        ...
                        logger.warning(
                            f"   No tools in result or result.tools is empty"
                        )
            ...
            logger.debug(
                "⚠️ _mcp_list_tools method not found on FastMCP (this is normal for some versions)"
            )
        ...
        if hasattr(original_mcp_server, "list_tools"):
            ...
            logger.debug(
                "⚠️ list_tools method not found on LowLevelServer (this is normal for some versions)"
            )
        ...
        if hasattr(self.mcp, "list_tools"):
            ...
            async def debug_fastmcp_list_tools(request):
                ...
                try:
                    ...
                    if result and isinstance(result, list):
                        logger.info(
                            f"   Converting {len(result)} FunctionTool objects to MCP format..."
                        )
                        ...
                        try:
                            ...
                            for tool in result:
                                if hasattr(tool, "name") and hasattr(
                                    ...
                                    tool_def = None
                                    if hasattr(self.mcp, "_tool_manager") and hasattr(
                                        self.mcp._tool_manager, "_tools"
                                    ):
                                        for (
                                            name,
                                            tool_obj,
                                        ) in self.mcp._tool_manager._tools.items():
                                            if name == tool.name:
                                                tool_def = tool_obj
                                                break
                                    ...
                                    logger.info(
                                        f"   Converted tool: {mcp_tool.name} with inputSchema type: {type(mcp_tool.inputSchema)}"
                                    )
                                    logger.info(
                                        f"   Full inputSchema: {mcp_tool.inputSchema}"
                                    )
                                    logger.info(f"   Full MCP Tool: {mcp_tool}")

                            logger.info(
                                f"   Converted {len(mcp_tools)} tools to proper MCP Tool objects"
                            )

                            # Try returning in FastMCP's native format but with our custom schemas
                            logger.info(
                                f"🧪 TESTING: Returning FastMCP native format with custom schemas"
                            )
                            logger.info(f"   Tools count: {len(mcp_tools)}")
                            logger.info(
                                "   Using FastMCP native format that might have been working before"
                            )
                            ...
                            logger.info(
                                f"   Created {len(fastmcp_tools)} FastMCP FunctionTool objects with custom schemas"
                            )
                            ...
                            logger.info(
                                f"   Created {len(mcp_tools)} dictionary tools as fallback"
                            )
                            return mcp_tools
                    else:
            ...
            logger.debug(
                "⚠️ list_tools method not found on FastMCP (this is normal for some versions)"
            )
        ...
        if hasattr(original_mcp_server, "call_tool"):
            ...
            logger.debug(
                "⚠️ call_tool method not found on LowLevelServer (this is normal for some versions)"
            )
        ...
        if hasattr(self.mcp, "call_tool"):
            ...
            logger.debug(
                "⚠️ call_tool method not found on FastMCP (this is normal for some versions)"
            )
        ...
        if hasattr(original_mcp_server, "handle_request"):
            ...
            logger.info(
                f"🔧 Found handle_request on LowLevelServer: {original_handle_request}"
            )
            ...
            logger.debug(
                "⚠️ handle_request method not found on LowLevelServer (this is normal for some versions)"
            )
    ...
    def register_tools(self):
        ...
        # Search records tool
        async def search_records(
            query: str = None, table: str = "incident", limit: int = 10, ctx=None
        ):
            """Search for records in ServiceNow.
        ...
        async def create_incident(
            short_description: str, description: str = None, priority: int = 3, ctx=None
        ):
            """Create a new incident in ServiceNow
        ...
        async def sn_search_incidents(query: str = "", limit: int = 10, ctx=None):
            ...
            if not query or query.strip() in {
                ...
                return await search_records(
                    query="", table="incident", limit=limit, ctx=ctx
                )

            if query and not any(
                m in query
                for m in ["=", "^", ".", "LIKE", "STARTSWITH", "ENDSWITH", ">", "<"]
            ):
                ...
                return await search_records(
                    query=enc, table="incident", limit=limit, ctx=ctx
                )
            return await search_records(
                query=query, table="incident", limit=limit, ctx=ctx
            )

        async def sn_search_user(query: str, limit: int = 10, ctx=None):
            """Search ServiceNow users by name/username/email (sys_user)."""
            if query and not any(
                m in query for m in ["=", "^", ".", "LIKE", "STARTSWITH", "ENDSWITH"]
            ):
                import urllib.parse
                ...
                return await search_records(
                    query=enc, table="sys_user", limit=limit, ctx=ctx
                )
            return await search_records(
                query=query, table="sys_user", limit=limit, ctx=ctx
            )

        async def sn_list_users(limit: int = 10, ctx=None):
            return await search_records(
                query="", table="sys_user", limit=limit, ctx=ctx
            )

        async def sn_get_incident(number: str, ctx=None):
            # Minimal wrap: encoded query by number
            return await search_records(
                query=f"number={number}", table="incident", limit=1, ctx=ctx
            )
        ...
        async def sn_get_user_incidents(username: str, limit: int = 25, ctx=None):
            ...
            if any(sep in username for sep in [".", "_"]):
                _add_variation(
                    username.replace(".", " ").replace("_", " "), username_variations
                )
                _add_variation(
                    username.replace(".", "").replace("_", ""), username_variations
                )
            if " " in username:
                ...
                _add_variation(
                    " ".join(p.capitalize() for p in parts), username_variations
                )
                _add_variation("".join(parts), username_variations)
            ...
            users = await search_records(
                query=user_enc, table="sys_user", limit=5, ctx=ctx
            )
            ...
            for i, u in enumerate(user_records):
                print(
                    f"🔍 User {i+1}: name='{u.get('name')}', user_name='{u.get('user_name')}', email='{u.get('email')}', sys_id='{u.get('sys_id')}'"
                )
            ...
            if not user_records:
                ...
                for alt in username_variations[1:]:
                    ...
                    alt_users = await search_records(
                        query=alt_query, table="sys_user", limit=5, ctx=ctx
                    )
            ...
            if not user_records:
                print(
                    f"🔍 No users found after variations, trying incident content search as fallback"
                )
                fallback_result = {
                    "success": True,
                    "count": 0,
                ...
                for term in username_variations:
                    ...
                    fallback_result = await search_records(
                        query=content_query, table="incident", limit=limit, ctx=ctx
                    )
                    print(f"🔍 Incident content search result: {fallback_result}")
                    if isinstance(fallback_result, dict) and fallback_result.get(
                        "records"
                    ):
                        print(
                            f"🔍 Found incidents via content search variation '{term}'"
                        )
            ...
            if ors:
                ...
                result = await search_records(
                    query=inc_q, table="incident", limit=limit, ctx=ctx
                )
                ...
                if isinstance(result, dict) and result.get("records"):
                    print(
                        f"🔍 Sys_id search successful, returning {len(result.get('records', []))} incidents"
                    )
                    return result
                else:
                    print(
                        f"🔍 Sys_id search returned no results, trying dot-walk fallback"
                    )
            else:
            ...
            if dotwalk_queries:
                ...
                fallback_result = await search_records(
                    query=dot_q, table="incident", limit=limit, ctx=ctx
                )
        ...
        async def sn_update_incident(
            ...
            try:
                ...
                if not target_sys_id:
                    raise ToolError(
                        "sn_update_incident requires either sys_id or a valid incident number"
                    )
                ...
                url = (
                    f"{self.client.instance_url}/api/now/table/incident/{target_sys_id}"
                )
                timeout = aiohttp.ClientTimeout(total=60)
                async with aiohttp.ClientSession(
    ...
    def run(
        ...
        """Run the MCP server"""
        logger.info(
            f"🚀 Starting ServiceNow MCP server with {transport.upper()} transport"
        )
        ...
        if transport == "http":
            ...
            logger.info(
                "🔧 FORCING PURE HTTP TRANSPORT - bypassing FastMCP's transport system"
            )
                    ...
                    try:
                        ...
                        if method == "initialize":
                            ...
                            try:
                                ...
                                if hasattr(mcp_server.mcp, "_tool_manager") and hasattr(
                                    mcp_server.mcp._tool_manager, "_tools"
                                ):
                                    tools = list(
                                        mcp_server.mcp._tool_manager._tools.values()
                                    )
                                    logger.info(
                                        f"   Found {len(tools)} tools in tool manager"
                                    )
                            ...
                            try:
                                if hasattr(mcp_server.mcp, "_tool_manager") and hasattr(
                                    ...
                                    if tool_name in mcp_server.mcp._tool_manager._tools:
                                        ...
                                        if hasattr(tool, "fn"):
                                            logger.info(
                                                f"   Executing tool function: {tool.fn}"
                                            )
                                            ...
                                            logger.info(
                                                f"   Raw ServiceNow result: {result}"
                                            )
                                            ...
                                            logger.info(
                                                "   Returning MCP-compliant result with content items: "
                                            ...
                                            logger.error(
                                                f"   Tool {tool_name} has no fn attribute"
                                            )
                                            return JSONResponse(
                                                content={
                                                    "jsonrpc": "2.0",
                                        ...
                                        logger.error(
                                            f"   Tool {tool_name} not found in tool manager"
                                        )
                                        return JSONResponse(
                                            content={
                                                "jsonrpc": "2.0",
                            ...
                            logger.info(
                                "🔧 NOTIFICATIONS/INITIALIZED REQUEST - acknowledging"
                            )
            ...
            async def handle_mcp_get():
                ...
                return JSONResponse(
                    content={"status": "ServiceNow MCP Server is running"}
                )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        instance_url = os.getenv(
            "SERVICENOW_INSTANCE_URL", "https://dev355156.service-now.com"
        )
