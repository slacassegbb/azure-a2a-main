...
class NLPProcessor:
    ...
    def parse_search_query(query: str) -> Dict[str, Any]:
        ...
        return {"table": table, "query": query_string, "limit": 10}
    ...
    def parse_update_command(command: str) -> Tuple[str, Dict[str, Any]]:
        ...
        number_match = re.search(
            r"(INC\d+|PRB\d+|CHG\d+|TASK\d+)", command, re.IGNORECASE
        )
        if not number_match:
    ...
    def parse_create_incident_command(command: str) -> Dict[str, Any]:
        ...
        if about_match:
            ...
            incident_data["short_description"] = description[:50] + (
            ...
            desc_match = re.search(
                r"(?:create|open|new)\s+(?:an?\s+)?(?:incident|ticket)\s+([^,\.]+)",
                command,
                re.IGNORECASE,
            )
            if desc_match:
                ...
                incident_data["description"] = description
                logger.debug(
                    f"Extracted description (alternate pattern): {description}"
                )

        # Extract urgency/priority
        if re.search(
            r"\b(urgent|critical|high\s+priority|high\s+urgency)\b",
            command,
            re.IGNORECASE,
        ):
            ...
            r"\b(medium\s+priority|medium\s+urgency|normal)\b", command, re.IGNORECASE
        ):
        ...
        if re.search(
            r"\b(affecting\s+all\s+users|company-wide|everyone|global)\b",
            command,
            re.IGNORECASE,
        ):
            incident_data["impact"] = 1  # High
            ...
            r"\b(multiple\s+users|department|team)\b", command, re.IGNORECASE
        ):
            incident_data["impact"] = 2  # Medium
            logger.debug("Set medium impact based on affected users")
        elif re.search(
            r"\b(single\s+user|one\s+person|individual)\b", command, re.IGNORECASE
        ):
            incident_data["impact"] = 3  # Low
        ...
        category_match = re.search(
            r"\b(hardware|software|network|security|database|email)\b",
            command,
            re.IGNORECASE,
        )
        if category_match:
