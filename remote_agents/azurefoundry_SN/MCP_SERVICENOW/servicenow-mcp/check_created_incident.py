            ...
            if hasattr(result, "content") and result.content:
                ...
                try:
                    ...
                    if response_data.get("status") == "success":
                        ...
                        print("-" * 30)
                        print(
                            f"📋 Incident Number: {metadata.get('number', 'Not available')}"
                        )
                        print(
                            f"🆔 System ID: {incident_data.get('sys_id', 'Not available')}"
                        )
                        print(
                            f"📝 Short Description: {incident_data.get('short_description', 'Not available')}"
                        )
                        print(
                            f"📄 Description: {incident_data.get('description', 'Not available')}"
                        )
                        print(
                            f"⚠️  Priority: {incident_data.get('priority', 'Not available')}"
                        )
                        print(
                            f"🔥 Urgency: {incident_data.get('urgency', 'Not available')}"
                        )
                        print(
                            f"💥 Impact: {incident_data.get('impact', 'Not available')}"
                        )
                        print(
                            f"📅 Created: {incident_data.get('sys_created_on', 'Not available')}"
                        )
                        print(
                            f"👤 Created By: {incident_data.get('sys_created_by', 'Not available')}"
                        )
                        ...
                        print(
                            f"3. Search for incident number: {metadata.get('number', 'N/A')}"
                        )
                        ...
                        if metadata.get("number"):
                            ...
                            if (
                                hasattr(search_result, "content")
                                and search_result.content
                            ):
                                ...
                                if metadata.get("number") in search_text:
                                    ...
                                    print(
                                        "⚠️  Could not verify incident in search results"
                                    )
                            else:
