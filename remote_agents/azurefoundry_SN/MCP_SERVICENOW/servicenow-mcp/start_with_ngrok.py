...
def start_mcp_server():
    ...
    try:
        ...
        if not all([username, password, instance_url]):
            logger.error(
                "Missing required environment variables: SERVICENOW_USERNAME, SERVICENOW_PASSWORD, SERVICENOW_INSTANCE_URL"
            )
            return
...
def setup_ngrok_tunnel():
    ...
    try:
        ...
        logger.error(
            "Make sure ngrok is properly installed and you have internet connection"
        )
        return None
