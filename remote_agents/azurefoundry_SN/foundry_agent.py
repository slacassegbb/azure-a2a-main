...
from azure.ai.agents.models import (Agent, AgentThread, BingGroundingTool,
                                    FilePurpose, FileSearchTool, ListSortOrder,
                                    McpTool, ThreadMessage, ThreadRun,
                                    ToolApproval)
...
class FoundrySNAgent:
    ...
    async def _setup_file_search(
        ...
        async with FoundrySNAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundrySNAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="shared_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundrySNAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[FoundrySNAgent._shared_vector_store.id]
                )
                logger.info(
    ...
    async def create_agent(self) -> Agent:
        ...
        try:
                ...
                try:
                    ...
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        async with client.stream(
                            "GET", "https://agent1.ngrok.app/mcp/"
                        ) as response:
                            logger.info(
                                f"   MCP Server Response: {response.status_code}"
                            )
                            logger.info(
                            ...
                            if response.status_code == 200:
                                ...
                                if (
                                    ...
                                    logger.info(
                                        "✅ MCP Server connectivity test PASSED (SSE endpoint detected)"
                                    )
                                    return True
                                else:
                                    ...
                                    try:
                                        ...
                                        async for chunk in response.aiter_text():
                                            content_chunk += chunk
                                            if (
                                                len(content_chunk) > 100
                                            ):  # Just read first 100 chars
                                                break
                                        logger.info(
                                            f"   Response Content (first 100 chars): {content_chunk[:100]}"
                                        )
                                        logger.info(
                                        ...
                                        logger.info(
                                            "✅ MCP Server connectivity test PASSED (SSE stream detected)"
                                        )
                                        return True
                            else:
                                logger.error(
                    ...
                    logger.info(
                        "✅ MCP Server connectivity test PASSED (SSE stream timeout - expected)"
                    )
                    return True
                except Exception as e:
            ...
            logger.info(
                f"   Tool definitions: {[str(t) for t in tools[:3]] if tools else 'None'}"
            )
            logger.info(f"   Tool resources: {tool_resources}")
            logger.info(
                "🔧 FIXED: Using correct tool_resources format with server_label as key"
            )
        ...
        logger.info(
            "🔧 REMOVED Salesforce tools - forcing Azure to use ServiceNow MCP tools"
        )
...
"""
    ...
    async def run_conversation_stream(self, thread_id: str, user_message: str):
        ...
        try:
            ...
            if inc_match:
                ...
                if _re.search(
                    r"\b(incidents? for|\bshow (me )?.*incidents? (for|of))\b",
                    user_message,
                ...
                try:
                    name_match = _re.search(
                        r"Customer\s+([A-Z][a-z]+\s+[A-Z][a-z]+)", user_message
                    )
                    keyword_present = (
                        _re.search(r"\bVPN\b", user_message, flags=_re.IGNORECASE)
                        is not None
                    )
                    wants_update = (
                        _re.search(
                            r"\bupdate\b|\bwork note\b|\bset (it|state) to\b",
                            user_message,
                            flags=_re.IGNORECASE,
                        )
                        is not None
                    ...
                    mentions_incidents = (
                        _re.search(
                            r"\bincidents?\b", user_message, flags=_re.IGNORECASE
                        )
                        is not None
                    )

                    if (name_match and mentions_incidents) or (
                        keyword_present and mentions_incidents
                    ):
                        person = (
                            name_match.group(1) if name_match else "the specified user"
                        )
                        ...
                        if wants_update:
                            compound_hint += f"Then choose the most recent open incident that matches '{kw or 'the topic'}' and call sn_update_incident to add work_notes and set state to 'In Progress' if currently 'New'."
        ...
        while (
            ...
            if run.status == "failed":
                ...
                try:
                    ...
                    logger.error(
                        f"   Total run steps: {len(run_steps.data) if hasattr(run_steps, 'data') else 'unknown'}"
                    )
                    ...
                    if hasattr(run_steps, "data"):
                        ...
                        logger.error(
                            f"   Using run_steps.data: {len(steps_list)} steps"
                        )
                    elif hasattr(run_steps, "__iter__"):
                        steps_list = list(run_steps)
                        logger.error(
                            f"   Using list(run_steps): {len(steps_list)} steps"
                        )
                    else:
                    ...
                    for i, step in enumerate(steps_list):
                        ...
                        logger.error(
                            f"     Created at: {getattr(step, 'created_at', 'N/A')}"
                        )
                            ...
                            if hasattr(details, "tool_calls") and details.tool_calls:
                                ...
                                for j, tool_call in enumerate(details.tool_calls):
                                    ...
                                    if hasattr(tool_call, "function"):
                                        ...
                                        logger.error(
                                            f"         Function name: {getattr(func, 'name', 'N/A')}"
                                        )
                                        logger.error(
                                            f"         Function args: {getattr(func, 'arguments', 'N/A')}"
                                        )

                                    # Check for MCP-specific details
                                    if getattr(tool_call, "type", None) == "mcp":
                                        logger.error(
                                            f"         🔧 MCP TOOL CALL DETECTED!"
                                        )
                                        logger.error(
                                            f"         MCP tool call object: {tool_call}"
                                        )
                    ...
                    logger.error(
                        f"   Step analysis traceback: {traceback.format_exc()}"
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"   Run status: {getattr(run, 'status', 'unknown')}"
                        )
                        logger.error(
                            f"   Run failed_at: {getattr(run, 'failed_at', 'unknown')}"
                        )
                        logger.error(
                            f"   Run started_at: {getattr(run, 'started_at', 'unknown')}"
                        )
                        logger.error(
                            f"   Run created_at: {getattr(run, 'created_at', 'unknown')}"
                        )
                        ...
                        if hasattr(run, "last_error") and run.last_error:
                            ...
                            logger.error(
                                f"   Error code: {getattr(error, 'code', 'N/A')}"
                            )
                            logger.error(
                                f"   Error message: {getattr(error, 'message', 'N/A')}"
                            )
                    ...
                    logger.error(
                        f"🔧 HYPOTHESIS: Azure Foundry failed during MCP tool execution"
                    )
                    logger.error(
                        f"   This suggests the McpTool could not communicate with the MCP server"
                    )
                    logger.error(f"   Even though initial connectivity test passed")
                    logger.error(
                        f"   Check MCP server logs for any requests during this timeframe"
                    )
    ...
    async def _handle_tool_calls(self, run: ThreadRun, thread_id: str):
        ...
        if (
            hasattr(required_action, "submit_tool_outputs")
            and required_action.submit_tool_outputs
        ):
            action_type = "submit_tool_outputs"
            tool_calls = (
                getattr(required_action.submit_tool_outputs, "tool_calls", []) or []
            )
        elif (
            hasattr(required_action, "submit_tool_approval")
            and required_action.submit_tool_approval
        ):
            action_type = "submit_tool_approval"
            tool_calls = (
                getattr(required_action.submit_tool_approval, "tool_calls", []) or []
            )
        else:
            logger.warning(
        ...
        if not tool_calls:
            logger.warning(
                "Required action contained no tool calls; nothing to process"
            )
            return
        ...
        if action_type == "submit_tool_outputs":
            ...
            for tool_call in tool_calls:
                try:
                    ...
                    logger.info(
                        "Processing tool output call %s with args %s",
                        function_name,
                        arguments,
                    )
                    ...
                    logger.error(
                        "Error constructing tool output for call %s: %s",
            ...
            if tool_outputs:
                ...
                logger.warning(
                    "No tool outputs generated; submitting empty acknowledgements"
                )
                fallback_outputs = [
        ...
        for tool_call in tool_calls:
            try:
                approvals.append(
                    ToolApproval(tool_call_id=tool_call.id, approve=True, headers={})
                )
                logger.info("Prepared approval for tool call %s", tool_call.id)
            except Exception as exc:
                logger.error(
                    "Could not prepare approval for tool call %s: %s",
                    getattr(tool_call, "id", "?"),
                    exc,
                )
                        ...
                        if (
                            ...
                            if function_name == "sf_create_case":
                                ...
                                if action == "block_card":
                                    card_num = args.get("parameters", {}).get(
                                        "card_number", "XXXX"
                                    )
                                    ...
                                    amount = args.get("parameters", {}).get(
                                        "amount", "$0.00"
                                    )
                                    merchant = args.get("parameters", {}).get(
                                        "merchant_name", "Unknown"
                                    )
                                    return (
                                        f"Creating dispute for {amount} at {merchant}"
                                    )
