...
Please process this human response and continue the workflow accordingly."""
            ...
            if responses:
                ...
                # No explicit text response — but if tools ran successfully, treat as success
                import uuid
                if tools_called and last_processing_event:
                    fallback_response = last_processing_event
                    logger.info(f"📤 No text response but tools succeeded, using last event: {fallback_response[:200]}")
                    message_parts = [TextPart(text=fallback_response)]
                    if hasattr(agent, 'last_token_usage') and agent.last_token_usage:
