...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    async def health_check(_: Request) -> PlainTextResponse:
        return PlainTextResponse('Image Generator Agent is running!')
...
def run_a2a_server_in_thread(host: str, port: int):
    ...
    print(f"Starting Image Generator Agent A2A server on {host}:{port}...")
...
async def launch_ui(host: str = "0.0.0.0", ui_port: int = DEFAULT_UI_PORT, a2a_port: int = DEFAULT_PORT):
    ...
    print("Starting Image Generator Agent with both UI and A2A server...")
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="Image Generator Agent") as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
            height=100,
    ...
    print(f"Launching Image Generator Agent Gradio interface on {host}:{ui_port}...")
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
    )
    print("Image Generator Agent Gradio application has been shut down.")
...
def cli(host: str, port: int, ui: bool, ui_port: int):
    """Image Generator Agent - run as an A2A server or with Gradio UI + A2A server."""
    if ui:
