...
async def main() -> None:
    """
    Test client for the AI Foundry Fraud Intelligence Agent A2A demo.
    ...
    base_url = os.getenv('A2A_BASE_URL', 'http://localhost:9004')
    logger.info(f"🔗 Connecting to AI Foundry Fraud Intelligence Agent at: {base_url}")
    ...
    async with httpx.AsyncClient(timeout=30.0) as httpx_client:
        ...
        try:
            ...
            logger.info("💡 Make sure the AI Foundry Fraud Intelligence Agent server is running:")
        ...
        # Test fraud-focused queries
        fraud_test_messages = [
            "Low-speed collision with identical medical notes—highlight fraud indicators.",
            "Home fire soon after coverage increase—summarize red flags and recommended escalation.",
            "Travel cancellation with questionable medical documentation—assess authenticity.",
            "Provider billing dozens of identical procedures—recommend fraud actions.",
            "List steps before escalating to SIU when multiple red flags are present."
        ]

        logger.info(f"\n🧪 Testing {len(fraud_test_messages)} fraud-focused queries:")

        for i, test_message in enumerate(fraud_test_messages, 1):
            logger.info(f"\n--- Test {i}/{ len(fraud_test_messages)} ---")
        ...
        logger.info("\n🎉 Fraud agent testing completed!")
        logger.info("📊 Test Summary:")
        logger.info(f"   - Agent: {final_agent_card_to_use.name}")
        logger.info(f"   - Base URL: {base_url}")
        logger.info(f"   - Test Messages: {len(fraud_test_messages)}")
...
if __name__ == '__main__':
    ...
    print("🤖 AI Foundry Fraud Intelligence Agent - A2A Test Client")
    print("=" * 50)
    print("This client tests the Agent-to-Agent communication")
    print("with our AI Foundry Fraud Intelligence Agent.")
