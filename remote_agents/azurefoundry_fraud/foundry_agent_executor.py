    ...
    async def execute(
        ...
        async with FoundryFraudAgentExecutor._request_semaphore:
            ...
            if FoundryFraudAgentExecutor._api_call_count >= FoundryFraudAgentExecutor._max_api_calls_per_minute:
                logger.warning(f"⚠️ API call count ({FoundryFraudAgentExecutor._api_call_count}) at limit, requests may be throttled by Azure")
            
            FoundryFraudAgentExecutor._api_call_count += 1
