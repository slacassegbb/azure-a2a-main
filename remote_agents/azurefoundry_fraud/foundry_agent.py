    ...
    async def _setup_file_search(
        ...
        async with FoundryFraudAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryFraudAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="shared_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryFraudAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[FoundryFraudAgent._shared_vector_store.id]
                )
                logger.info(
