...
"""
    ...
    async def cleanup_shared_resources(cls):
        """Clean up shared fraud knowledge document search resources (call when shutting down completely)."""
        try:
            if cls._shared_vector_store or cls._shared_uploaded_files:
                ...
                logger.info("Shared fraud document search resources cleaned up")
                
        except Exception as e:
            logger.error(f"Error cleaning up shared fraud document search resources: {e}")
...
async def create_foundry_fraud_agent() -> FoundryFraudAgent:
    """Factory function to create and initialize a Foundry Fraud Intelligence agent."""
...
async def demo_agent_interaction():
    """Demo function showing how to use the Foundry Fraud Intelligence agent for claim analysis."""
    agent = await create_foundry_fraud_agent()

    try:
        thread = await agent.create_thread()

        message = "Auto claim with low-speed collision and identical doctor notes—assess fraud risk factors."
        print(f"\nUser: {message}")
        ...
        # await FoundryFraudAgent.cleanup_shared_resources()
...
def test_citation_improvement():
    ...
    test_response = """To process the auto collision estimate, follow the steps below:
Coverage Confirmation:
Verify the inspection notes include labor hours and parts pricing per auto_estimation.md section 2.
Estimate Calculation:
Apply the $1,000 collision deductible guidance and confirm repairs stay below the 75% total loss threshold.
Rental Allowance:
Recommend rental coverage based on 30-day allowance from auto_estimation.md section 2.
Documentation:
Collect collision photos, repair estimate, and odometer reading as listed in procedures_faq.md section 1.
Compliance:
Ensure severity classification follows universal_estimation.md section 2 and note any escalation triggers from procedures_faq.md section 2.
Following these actions keeps the estimate aligned with policy guidelines and QA standards."""
