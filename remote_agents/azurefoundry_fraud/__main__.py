...
from foundry_agent_executor import (create_foundry_agent_executor,
                                    initialize_foundry_fraud_agents_at_startup)
...
def run_a2a_server_in_thread(host: str, port: int):
    ...
    print(
        f"Starting AI Foundry Fraud Intelligence Agent A2A server on {host}:{port}..."
    )
...
async def launch_ui(
    ...
    with gr.Blocks(
        theme=gr.themes.Ocean(), title="AI Foundry Fraud Intelligence Agent"
    ) as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
    ...
    print(
        f"Launching AI Foundry Fraud Intelligence Agent Gradio interface on {host}:{ui_port}..."
    )
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
