...
class FoundryClassificationAgent:
            ...
            try:
                ...
                for file_path in file_paths:
                    try:
                        ...
                        FoundryClassificationAgent._shared_uploaded_files.append(
                            file.id
                        )
                        logger.info(
                ...
                FoundryClassificationAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="shared_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryClassificationAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[
                        FoundryClassificationAgent._shared_vector_store.id
                    ]
                )
                logger.info(
    ...
    async def create_agent(self) -> Agent:
        ...
        if self.agent:
            logger.info(
                "Classification agent already exists, returning existing instance"
            )
            return self.agent
        ...
        if self._file_search_tool:
            ...
            if hasattr(self._file_search_tool, "resources"):
                ...
                logger.info(
                    "Added file search tool resources for customer support documents"
                )
