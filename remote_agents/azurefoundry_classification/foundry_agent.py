...
"""
    ...
    async def cleanup_shared_resources(cls):
        """Clean up shared classification triage document search resources (call when shutting down completely)."""
        try:
            if cls._shared_vector_store or cls._shared_uploaded_files:
                ...
                logger.info("Shared classification triage document search resources cleaned up")
                
        except Exception as e:
            logger.error(f"Error cleaning up shared classification triage document search resources: {e}")
...
async def create_foundry_classification_agent() -> FoundryClassificationAgent:
    """Factory function to create and initialize a Foundry Classification Triage agent."""
    agent = FoundryClassificationAgent()
...
async def demo_agent_interaction():
    """Demo function showing how to use the Foundry Classification Triage agent for incident analysis."""
    agent = await create_foundry_classification_agent()
    ...
    try:
        ...
        # Example interaction
        message = "I lost my credit card while traveling in Europe yesterday and I'm worried someone might use it fraudulently."
        print(f"\nUser: {message}")
        ...
        # await FoundryClassificationAgent.cleanup_shared_resources()
...
Immediate Reporting:
Notify the bank immediately by calling their lost card hotline. For bank, the 24/7 number is 1-800-950-5114 (for U.S. bank credit cards)【4:0†source】.
...
Timely reporting ensures you are not held liable for unauthorized charges. Federal laws limit consumer liability to $50, but bank's policy is $0 for confirmed fraud cases【4:2†source】.
