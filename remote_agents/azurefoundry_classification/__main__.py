...
from foundry_agent_executor import (
    ...
    initialize_foundry_classification_agents_at_startup)
...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    async def health_check(_: Request) -> PlainTextResponse:
        return PlainTextResponse(
            "AI Foundry Insurance Classification Agent is running!"
        )
...
def run_a2a_server_in_thread(host: str, port: int):
    ...
    print(
        f"Starting AI Foundry Classification Triage Agent A2A server on {host}:{port}..."
    )
...
async def register_agent_with_host(agent_card):
    ...
    if SELF_REGISTRATION_AVAILABLE:
        ...
        try:
            logger.info(
                f"🤝 Attempting to register '{agent_card.name}' with host agent..."
            )
            registration_success = await register_with_host_agent(
                agent_card, host_url=HOST_AGENT_URL or None
            )
    ...
    try:
        ...
        if agent_executor_instance is None:
            ...
            agent_executor_instance = create_foundry_agent_executor(
                AgentCard(
                    name="UI Agent", description="UI Agent", url="", version="1.0.0"
                )
            )
        ...
        thread_id = await agent_executor_instance._get_or_create_thread(
            "ui_context", foundry_agent
        )
        ...
        async for response in foundry_agent.run_conversation_stream(thread_id, message):
            ...
            if isinstance(response, str):
                ...
                logger.debug(
                    f"get_foundry_response: Unexpected response type: {type(response)}"
                )
                yield gr.ChatMessage(
                    role="assistant",
...
async def launch_ui(
    ...
    print(
        "Starting AI Foundry Classification Triage Agent with both UI and A2A server..."
    )
    ...
    with gr.Blocks(
        theme=gr.themes.Ocean(), title="AI Foundry Classification Triage Agent"
    ) as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
    ...
    print(
        f"Launching AI Foundry Classification Triage Agent Gradio interface on {host}:{ui_port}..."
    )
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
    ...
    print(
        "AI Foundry Classification Triage Agent Gradio application has been shut down."
    )
