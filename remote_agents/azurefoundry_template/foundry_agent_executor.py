...
class FoundryTemplateAgentExecutor(AgentExecutor):
    ...
    async def _get_or_create_agent(self) -> FoundryTemplateAgent:
        ...
        async with FoundryTemplateAgentExecutor._agent_lock:
            if not FoundryTemplateAgentExecutor._shared_foundry_agent:
                ...
                FoundryTemplateAgentExecutor._shared_foundry_agent = (
                    FoundryTemplateAgent()
                )
    ...
    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryTemplateAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        async with FoundryTemplateAgentExecutor._request_semaphore:
            ...
            if (
                FoundryTemplateAgentExecutor._api_call_count
                >= FoundryTemplateAgentExecutor._max_api_calls_per_minute
            ):
                wait_time = 60 - (
                    current_time - FoundryTemplateAgentExecutor._api_call_window_start
                )
                if wait_time > 0:
                    logger.warning(
            ...
            time_since_last = (
                current_time - FoundryTemplateAgentExecutor._last_request_time
            )
            if time_since_last < FoundryTemplateAgentExecutor._min_request_interval:
                sleep_time = (
                    FoundryTemplateAgentExecutor._min_request_interval - time_since_last
                )
                logger.warning(
