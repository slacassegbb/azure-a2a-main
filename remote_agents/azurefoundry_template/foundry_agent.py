    ...
    async def _setup_file_search(
        ...
        async with FoundryTemplateAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryTemplateAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="agent_template_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryTemplateAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[FoundryTemplateAgent._shared_vector_store.id]
                )
                logger.info(
