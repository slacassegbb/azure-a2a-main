...
def _resolve_default_ui_port() -> int:
    """Resolve UI port from environment variable or use default."""
    raw_port = _normalize_env_value(os.getenv('GRADIO_SERVER_PORT'))
    if raw_port:
        try:
            return int(raw_port)
        except ValueError:
            logger.warning("Invalid GRADIO_SERVER_PORT value '%s'; defaulting to 9120", raw_port)
    return 9120

DEFAULT_UI_PORT = _resolve_default_ui_port()  # Default UI port for Gradio interface
...
def _build_agent_skills() -> List[AgentSkill]:
    ...
    return [
        # EXAMPLE SKILL - Replace with your own skills
         AgentSkill(
            id='apcalc',
            name='AP-Calculus Study Buddy',
            description="This agent knows the AP-Calculus curriculum and can help students study for the exam by providing explanations, answering questions, and offering practice problems.",
            tags=['apcalculus', 'math', 'study', 'practice problems'],
            examples=[
                'Example query 1 - Create a study guide for chapter 5 in AP-Calculus.',                
            ],
        ),
...
def _create_agent_card(host: str, port: int) -> AgentCard:
    ...
    return AgentCard(
        name='Benjamin School AP-Calculus Study Buddy',  # ⚠️ CHANGE THIS to your agent's name
        description="This agent knows the AP-Calculus curriculum and can help students study for the exam by providing explanations, answering questions, and offering practice problems.",  # ⚠️ CHANGE THIS
...
def create_a2a_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    async def health_check(_: Request) -> PlainTextResponse:
        return PlainTextResponse('Benjamin School AP-Calculus Study Buddy is running!')  # ⚠️ CUSTOMIZATION: Update agent name
