    ...
    async def execute(
        ...
        async with FoundryDeepSearchAgentExecutor._request_semaphore:
            ...
            if FoundryDeepSearchAgentExecutor._api_call_count >= FoundryDeepSearchAgentExecutor._max_api_calls_per_minute:
                wait_time = 60 - (current_time - FoundryDeepSearchAgentExecutor._api_call_window_start)
                if wait_time > 0:
            ...
            time_since_last = current_time - FoundryDeepSearchAgentExecutor._last_request_time
            if time_since_last < FoundryDeepSearchAgentExecutor._min_request_interval:
                sleep_time = FoundryDeepSearchAgentExecutor._min_request_interval - time_since_last
