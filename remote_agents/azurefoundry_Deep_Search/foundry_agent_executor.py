...
class FoundryDeepSearchAgentExecutor(AgentExecutor):
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    ...
                    logger.info(
                        "✅ Foundry Deep Search agent startup initialization completed successfully"
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Failed to initialize deep search agent at startup: {e}"
                    )
        ...
        async with FoundryDeepSearchAgentExecutor._agent_lock:
            if not FoundryDeepSearchAgentExecutor._shared_foundry_agent:
                if FoundryDeepSearchAgentExecutor._startup_complete:
                    ...
                    raise RuntimeError(
                        "Deep search agent startup initialization failed - agent not available"
                    )

                # Fallback to lazy creation if startup wasn't called
                logger.warning(
                    "⚠️ Deep search agent not initialized at startup, falling back to lazy creation..."
                )
                FoundryDeepSearchAgentExecutor._shared_foundry_agent = (
                    FoundryDeepSearchAgent()
                )
    ...
    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryDeepSearchAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        async with FoundryDeepSearchAgentExecutor._request_semaphore:
            ...
            if (
                current_time - FoundryDeepSearchAgentExecutor._api_call_window_start
                > 60
            ):
            ...
            if (
                FoundryDeepSearchAgentExecutor._api_call_count
                >= FoundryDeepSearchAgentExecutor._max_api_calls_per_minute
            ):
                wait_time = 60 - (
                    current_time - FoundryDeepSearchAgentExecutor._api_call_window_start
                )
                if wait_time > 0:
                    ...
                    # Reset counters
                    FoundryDeepSearchAgentExecutor._api_call_count = 0
                    FoundryDeepSearchAgentExecutor._api_call_window_start = time.time()

            # Enforce minimum interval between requests - THIS IS THE KEY FIX
            time_since_last = (
                current_time - FoundryDeepSearchAgentExecutor._last_request_time
            )
            if time_since_last < FoundryDeepSearchAgentExecutor._min_request_interval:
                sleep_time = (
                    FoundryDeepSearchAgentExecutor._min_request_interval
                    - time_since_last
                )
                logger.warning(
