...
class FoundryTemplateAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        try:
            ...
            # Check for file artifacts (downloaded email attachments)
            artifacts = agent.pop_latest_artifacts()
            artifact_parts = []
            if artifacts:
                logger.info(f"📎 Found {len(artifacts)} attachment artifacts to return")
                # Convert artifacts to FilePart with FileWithUri
                for artifact in artifacts:
                    if isinstance(artifact, dict) and artifact.get("artifact-uri"):
                        file_with_uri = FileWithUri(
                            name=artifact.get("file-name", "attachment"),
                ...
                # Send artifact message immediately (so downstream agents can use the files)
                if artifact_parts:
            ...
            if responses:
                ...
                # Build message parts with text, artifacts, and optional token usage
                import uuid
                final_parts = [Part(root=TextPart(text=final_response))]
                
                # Include artifact parts in final message for agent-to-agent file exchange
                if artifact_parts:
