...
class FoundryClaimsAgentExecutor(AgentExecutor):
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    ...
                    logger.info(
                        "✅ Foundry Claims agent startup initialization completed successfully"
                    )
        ...
        async with FoundryClaimsAgentExecutor._agent_lock:
            if not FoundryClaimsAgentExecutor._shared_foundry_agent:
                if FoundryClaimsAgentExecutor._startup_complete:
                    ...
                    raise RuntimeError(
                        "Claims agent startup initialization failed - agent not available"
                    )

                # Fallback to lazy creation if startup wasn't called
                logger.warning(
                    "⚠️ Claims agent not initialized at startup, falling back to lazy creation..."
                )
    ...
    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryClaimsAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        async with FoundryClaimsAgentExecutor._request_semaphore:
            ...
            if (
                FoundryClaimsAgentExecutor._api_call_count
                >= FoundryClaimsAgentExecutor._max_api_calls_per_minute
            ):
                wait_time = 60 - (
                    current_time - FoundryClaimsAgentExecutor._api_call_window_start
                )
                if wait_time > 0:
                    logger.warning(
            ...
            time_since_last = (
                current_time - FoundryClaimsAgentExecutor._last_request_time
            )
            if time_since_last < FoundryClaimsAgentExecutor._min_request_interval:
                sleep_time = (
                    FoundryClaimsAgentExecutor._min_request_interval - time_since_last
                )
                logger.warning(
