"""
AI Foundry Claims Specialist Agent Executor for A2A framework.
Adapted from ADK agent executor pattern to work with Azure AI Foundry agents for multi-line claims support.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryClaimsAgent
...
class FoundryClaimsAgentExecutor(AgentExecutor):
    """
    An AgentExecutor that runs Azure AI Foundry-based claims specialist agents for insurance claim management.
    Adapted from the ADK agent executor pattern.
    """

    # Class-level shared agent instance to avoid multiple agent creations
    _shared_foundry_agent: Optional[FoundryClaimsAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryClaimsAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        """Initialize the shared claims agent at startup instead of on first request."""
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                logger.info("🚀 Initializing Foundry Claims agent at startup...")
                try:
                    cls._shared_foundry_agent = FoundryClaimsAgent()
                    await cls._shared_foundry_agent.create_agent()
                    cls._startup_complete = True
                    logger.info("✅ Foundry Claims agent startup initialization completed successfully")
                except Exception as e:
                    logger.error(f"❌ Failed to initialize claims agent at startup: {e}")
    ...
    async def _get_or_create_agent(self) -> FoundryClaimsAgent:
        """Get the shared Foundry Claims agent (with fallback to lazy creation)."""
        async with FoundryClaimsAgentExecutor._agent_lock:
            if not FoundryClaimsAgentExecutor._shared_foundry_agent:
                if FoundryClaimsAgentExecutor._startup_complete:
                    # Startup was supposed to happen but failed
                    raise RuntimeError("Claims agent startup initialization failed - agent not available")

                # Fallback to lazy creation if startup wasn't called
                logger.warning("⚠️ Claims agent not initialized at startup, falling back to lazy creation...")
                FoundryClaimsAgentExecutor._shared_foundry_agent = FoundryClaimsAgent()
                await FoundryClaimsAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback claims agent creation completed")
            return FoundryClaimsAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryClaimsAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def _cleanup_thread_later(self, agent: FoundryClaimsAgent, thread_id: str):
    ...
    async def execute(
        ...
        async with FoundryClaimsAgentExecutor._request_semaphore:
            # Check API call rate limiting
            current_time = time.time()
            
            # Reset the window if it's been more than a minute
            if current_time - FoundryClaimsAgentExecutor._api_call_window_start > 60:
                FoundryClaimsAgentExecutor._api_call_count = 0
                FoundryClaimsAgentExecutor._api_call_window_start = current_time
            
            # Check if we're approaching the API limit
            if FoundryClaimsAgentExecutor._api_call_count >= FoundryClaimsAgentExecutor._max_api_calls_per_minute:
                wait_time = 60 - (current_time - FoundryClaimsAgentExecutor._api_call_window_start)
                if wait_time > 0:
                    logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                    await asyncio.sleep(wait_time)
                    # Reset counters
                    FoundryClaimsAgentExecutor._api_call_count = 0
                    FoundryClaimsAgentExecutor._api_call_window_start = time.time()
            
            # Enforce minimum interval between requests - THIS IS THE KEY FIX
            time_since_last = current_time - FoundryClaimsAgentExecutor._last_request_time
            if time_since_last < FoundryClaimsAgentExecutor._min_request_interval:
                sleep_time = FoundryClaimsAgentExecutor._min_request_interval - time_since_last
            ...
            FoundryClaimsAgentExecutor._last_request_time = time.time()
...
def create_foundry_agent_executor(card: AgentCard) -> FoundryClaimsAgentExecutor:
    return FoundryClaimsAgentExecutor(card)


async def create_foundry_agent_executor_with_startup(card: AgentCard) -> FoundryClaimsAgentExecutor:
    """Create claims executor and initialize the shared agent at startup."""
    await FoundryClaimsAgentExecutor.initialize_at_startup()
    return FoundryClaimsAgentExecutor(card)


async def initialize_foundry_claims_agents_at_startup():
    """
    Convenience function to initialize shared claims agent resources at application startup.
    ...
    import asyncio
    from foundry_agent_executor import initialize_foundry_claims_agents_at_startup

    async def startup():
        print("🚀 Starting claims application...")
        await initialize_foundry_claims_agents_at_startup()
        print("✅ Claims agent initialization complete, ready to handle requests")
