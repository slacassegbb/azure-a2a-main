...
async def main() -> None:
    """
    Test client for the AI Foundry Claims Specialist Agent A2A demo.
    Exercises both public and extended agent card functionality.
    """
    ...
    base_url = os.getenv('A2A_BASE_URL', 'http://localhost:9001')
    logger.info(f"🔗 Connecting to AI Foundry Claims Specialist Agent at: {base_url}")
    ...
    async with httpx.AsyncClient(timeout=30.0) as httpx_client:
        ...
        try:
            ...
            logger.info("💡 Make sure the AI Foundry Claims Specialist Agent server is running:")
        ...
        # Test claims-specific queries
        claims_test_messages = [
            "My sedan hit a deer and the repair estimate is $5,400. What will the insurance payout be after my $1,000 deductible?",
            "A pipe burst flooded our finished basement. What documentation do you need and am I covered?",
            "I canceled a $3,200 vacation because of a medical emergency—can travel insurance reimburse me?",
            "The hospital billed me $12,000 for inpatient care. How will my $1,500 deductible and 20% coinsurance apply?",
            "I suspect fraud on a recent claim. What compliance steps should I follow?"
        ]

        logger.info(f"\n🧪 Testing {len(claims_test_messages)} claims-oriented queries:")

        for i, test_message in enumerate(claims_test_messages, 1):
            logger.info(f"\n--- Test {i}/{ len(claims_test_messages)} ---")
        ...
        logger.info("\n🎉 Claims specialist agent testing completed!")
        logger.info("📊 Test Summary:")
        logger.info(f"   - Agent: {final_agent_card_to_use.name}")
        logger.info(f"   - Base URL: {base_url}")
        logger.info(f"   - Test Messages: {len(claims_test_messages)}")
...
if __name__ == '__main__':
    ...
    print("🤖 AI Foundry Claims Specialist Agent - A2A Test Client")
    print("=" * 50)
    print("This client tests the Agent-to-Agent communication")
    print("with our AI Foundry Claims Specialist Agent.")
