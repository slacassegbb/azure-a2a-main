...
class FoundryClaimsAgent:
    ...
    async def _setup_file_search(
        ...
        async with FoundryClaimsAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryClaimsAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="shared_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryClaimsAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[FoundryClaimsAgent._shared_vector_store.id]
                )
                logger.info(
    ...
    async def create_agent(self) -> Agent:
        ...
        if self.agent:
            logger.info(
                "Claims specialist agent already exists, returning existing instance"
            )
            return self.agent
