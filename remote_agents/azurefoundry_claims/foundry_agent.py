...
"""
    ...
    async def cleanup_shared_resources(cls):
        """Clean up shared claims knowledge document search resources (call when shutting down completely)."""
        try:
            if cls._shared_vector_store or cls._shared_uploaded_files:
                ...
                logger.info("Shared claims document search resources cleaned up")
                
        except Exception as e:
            logger.error(f"Error cleaning up shared claims document search resources: {e}")
...
async def create_foundry_claims_agent() -> FoundryClaimsAgent:
    """Factory function to create and initialize a Foundry Claims Specialist agent."""
...
async def demo_agent_interaction():
    """Demo function showing how to use the Foundry Claims Specialist agent for claims analysis."""
    ...
    try:
        ...
        # Example interaction
        message = "My basement flooded after a burst pipe and I need to know how the homeowners policy will respond."
        print(f"\nUser: {message}")
        ...
        # await FoundryClaimsAgent.cleanup_shared_resources()
...
def test_citation_improvement():
    ...
    test_response = """To process the auto collision claim, follow the steps below:
Coverage Confirmation:
Verify the policy includes AUTO-COLL and AUTO-RNT per auto_claims.md section 1.
Settlement Calculation:
Apply the $1,000 collision deductible and ensure the 75% total loss threshold from auto_claims.md section 2 is not exceeded.
Rental Allowance:
Authorize rental reimbursement up to $50 per day for 30 days (auto_claims.md section 1).
Documentation:
Collect collision report, repair estimate, and odometer reading per auto_claims.md section 3.
Compliance:
Ensure no exclusions from auto_claims.md section 4 apply and cross-check fraud indicators in regulatory_compliance.md section 2.
Following these actions keeps the payout aligned with policy terms and compliance expectations."""
