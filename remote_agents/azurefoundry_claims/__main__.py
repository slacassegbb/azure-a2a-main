...
from foundry_agent_executor import (
    create_foundry_agent_executor, initialize_foundry_claims_agents_at_startup)
...
async def launch_ui(
    ...
    with gr.Blocks(
        theme=gr.themes.Ocean(), title="AI Foundry Claims Specialist Agent"
    ) as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
    ...
    print(
        f"Launching AI Foundry Claims Specialist Agent Gradio interface on {host}:{ui_port}..."
    )
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
