"""
AI Foundry Agent Executor for A2A framework - Word Agent.
Handles artifact (file) delivery via the A2A protocol, mirroring
the pattern used by the PowerPoint agent.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryWordAgent
...
class FoundryAgentExecutor(AgentExecutor):
    _shared_foundry_agent: Optional[FoundryWordAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryWordAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                try:
                    cls._shared_foundry_agent = FoundryWordAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryWordAgent:
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundryWordAgent()
    ...
    async def _process_request(self, message_parts, context_id, task_updater, request_context=None):
        try:
            ...
            # --- Check for file artifacts (same pattern as PowerPoint agent) ---
                ...
                for artifact in artifacts:
                    if isinstance(artifact, dict) and artifact.get("artifact-uri"):
                        file_with_uri = FileWithUri(
                            name=artifact.get("file-name", "document.docx"),
                            uri=artifact["artifact-uri"],
                            mimeType=artifact.get("mime", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
                        )
            ...
            if responses:
                ...
                if artifact_parts:
                    final_parts = [Part(root=TextPart(text="Document created successfully."))]
                    final_parts.extend(artifact_parts)
                    if hasattr(agent, "last_token_usage") and agent.last_token_usage:
                        final_parts.append(Part(root=DataPart(data={"type": "token_usage", **agent.last_token_usage})))
                    await task_updater.complete(
