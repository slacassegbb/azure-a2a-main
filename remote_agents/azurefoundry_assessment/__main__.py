...
from foundry_agent_executor import (
    ...
    initialize_foundry_assessment_agents_at_startup,
)
...
def run_a2a_server_in_thread(host: str, port: int):
    ...
    print(
        f"Starting AI Foundry Assessment & Estimation Agent A2A server on {host}:{port}..."
    )
...
async def launch_ui(
    ...
    print(
        "Starting AI Foundry Assessment & Estimation Agent with both UI and A2A server..."
    )
    ...
    with gr.Blocks(
        theme=gr.themes.Ocean(), title="AI Foundry Assessment & Estimation Agent"
    ) as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
    ...
    print(
        f"Launching AI Foundry Assessment & Estimation Agent Gradio interface on {host}:{ui_port}..."
    )
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
    ...
    print(
        "AI Foundry Assessment & Estimation Agent Gradio application has been shut down."
    )
