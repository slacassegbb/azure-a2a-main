#!/usr/bin/env python3
"""
Test script for Azure Foundry Assessment & Estimation Agent self-registration

This script tests the self-registration functionality for the Azure AI Foundry
assessment agent to ensure it can automatically register with the host agent.
...
def create_test_agent_card(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT) -> AgentCard:
    ...
    skills = [
        AgentSkill(
            ...
            description="Provides labor/material cost breakdowns, severity ratings, and total-loss indicators for vehicle inspections.",
            tags=['auto', 'assessment', 'estimation'],
            examples=[
                'Front bumper and headlight replacement estimate',
                'Hail dents across hood and roof',
                'Flooded interior inspection with corrosion risk'
            ],
        ),
        AgentSkill(
            id='estimation_quality_assurance',
            name='Estimation Quality & QA',
            description="Applies universal estimation workflow, depreciation rules, and QA checklist for multi-line losses.",
            tags=['qa', 'workflow', 'depreciation'],
            examples=[
                'Apply severity matrix to loss scenario',
                'Checklist before submitting an estimate',
                'Explain depreciation for electronics and furniture'
            ],
        )
    ]
    ...
    return AgentCard(
        ...
        description="An intelligent assessment specialist powered by Azure AI Foundry that delivers cost estimates, severity ratings, and structured reports across auto, property, travel, and health scenarios.",
...
async def test_self_registration():
    ...
    print("🧪 Testing Azure Foundry Assessment & Estimation Agent Self-Registration")
    print("=" * 60)
...
if __name__ == "__main__":
    print("🚀 Azure Foundry Assessment & Estimation Agent Self-Registration Test")
    print()
