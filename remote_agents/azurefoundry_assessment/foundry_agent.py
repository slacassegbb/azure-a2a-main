    ...
    async def _setup_file_search(
        ...
        async with FoundryAssessmentAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryAssessmentAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="shared_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryAssessmentAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[FoundryAssessmentAgent._shared_vector_store.id]
                )
                logger.info(
...
async def demo_agent_interaction():
    ...
    try:
        ...
        # Example interaction
        message = (
            "My basement flooded after a burst pipe and I need an estimate for repairs."
        )
        print(f"\nUser: {message}")
