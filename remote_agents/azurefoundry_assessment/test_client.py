...
async def main() -> None:
    """
    Test client for the AI Foundry Assessment & Estimation Agent A2A demo.
    ...
    base_url = os.getenv('A2A_BASE_URL', 'http://localhost:9002')
    logger.info(f"🔗 Connecting to AI Foundry Assessment & Estimation Agent at: {base_url}")
    ...
    async with httpx.AsyncClient(timeout=30.0) as httpx_client:
        ...
        try:
            ...
            logger.info("💡 Make sure the AI Foundry Assessment & Estimation Agent server is running:")
        ...
        # Test assessment-specific queries
        assessment_test_messages = [
            "My sedan hit a deer and the labor shop quoted 20 hours plus parts. Estimate total repair cost.",
            "Basement flood damaged drywall and flooring. Provide replacement and labor cost breakdown.",
            "Trip was cancelled due to illness. Estimate reimbursable costs for a $3,200 itinerary.",
            "Hospital billed $12,000 for a 3-day stay. Provide typical cost ranges for comparison.",
            "Need QA checklist items before submitting an estimate report."
        ]

        logger.info(f"\n🧪 Testing {len(assessment_test_messages)} assessment-focused queries:")

        for i, test_message in enumerate(assessment_test_messages, 1):
            logger.info(f"\n--- Test {i}/{ len(assessment_test_messages)} ---")
        ...
        logger.info("\n🎉 Assessment agent testing completed!")
        logger.info("📊 Test Summary:")
        logger.info(f"   - Agent: {final_agent_card_to_use.name}")
        logger.info(f"   - Base URL: {base_url}")
        logger.info(f"   - Test Messages: {len(assessment_test_messages)}")
...
if __name__ == '__main__':
    ...
    print("🤖 AI Foundry Assessment & Estimation Agent - A2A Test Client")
    print("=" * 50)
    print("This client tests the Agent-to-Agent communication")
    print("with our AI Foundry Assessment & Estimation Agent.")
