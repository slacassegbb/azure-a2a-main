"""
AI Foundry Assessment & Estimation Agent Executor for A2A framework.
Adapted from ADK agent executor pattern to work with Azure AI Foundry agents for loss assessment and cost estimation.
"""
import asyncio
import logging
import base64
...
class FoundryAssessmentAgentExecutor(AgentExecutor):
    """
    An AgentExecutor that runs Azure AI Foundry-based assessment and estimation agents for multi-line damage evaluation.
    Adapted from the ADK agent executor pattern.
    """

    # Class-level shared agent instance to avoid multiple agent creations
    _shared_foundry_agent: Optional[FoundryAssessmentAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryAssessmentAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        """Initialize the shared assessment agent at startup instead of on first request."""
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                logger.info("🚀 Initializing Foundry Assessment agent at startup...")
                try:
                    cls._shared_foundry_agent = FoundryAssessmentAgent()
                    await cls._shared_foundry_agent.create_agent()
                    cls._startup_complete = True
                    logger.info("✅ Foundry Assessment agent startup initialization completed successfully")
                except Exception as e:
                    logger.error(f"❌ Failed to initialize assessment agent at startup: {e}")
    ...
    async def _get_or_create_agent(self) -> FoundryAssessmentAgent:
        ...
        async with FoundryAssessmentAgentExecutor._agent_lock:
            if not FoundryAssessmentAgentExecutor._shared_foundry_agent:
                if FoundryAssessmentAgentExecutor._startup_complete:
                    # Startup was supposed to happen but failed
                    raise RuntimeError("Assessment agent startup initialization failed - agent not available")

                # Fallback to lazy creation if startup wasn't called
                logger.warning("⚠️ Assessment agent not initialized at startup, falling back to lazy creation...")
                FoundryAssessmentAgentExecutor._shared_foundry_agent = FoundryAssessmentAgent()
                await FoundryAssessmentAgentExecutor._shared_foundry_agent.create_agent()
                logger.info("Fallback assessment agent creation completed")
            return FoundryAssessmentAgentExecutor._shared_foundry_agent

    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundryAssessmentAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        async with FoundryAssessmentAgentExecutor._request_semaphore:
            # Check API call rate limiting
            current_time = time.time()
            
            # Reset the window if it's been more than a minute
            if current_time - FoundryAssessmentAgentExecutor._api_call_window_start > 60:
                FoundryAssessmentAgentExecutor._api_call_count = 0
                FoundryAssessmentAgentExecutor._api_call_window_start = current_time
            
            # Check if we're approaching the API limit
            if FoundryAssessmentAgentExecutor._api_call_count >= FoundryAssessmentAgentExecutor._max_api_calls_per_minute:
                wait_time = 60 - (current_time - FoundryAssessmentAgentExecutor._api_call_window_start)
                if wait_time > 0:
                    logger.warning(f"API rate limit protection: waiting {wait_time:.1f}s to reset window")
                    await asyncio.sleep(wait_time)
                    # Reset counters
                    FoundryAssessmentAgentExecutor._api_call_count = 0
                    FoundryAssessmentAgentExecutor._api_call_window_start = time.time()
            
            # Enforce minimum interval between requests - THIS IS THE KEY FIX
            time_since_last = current_time - FoundryAssessmentAgentExecutor._last_request_time
            if time_since_last < FoundryAssessmentAgentExecutor._min_request_interval:
                sleep_time = FoundryAssessmentAgentExecutor._min_request_interval - time_since_last
            ...
            FoundryAssessmentAgentExecutor._last_request_time = time.time()
...
async def initialize_foundry_assessment_agents_at_startup():
    """
    Convenience function to initialize shared assessment agent resources at application startup.
    ...
    import asyncio
    from foundry_agent_executor import initialize_foundry_assessment_agents_at_startup

    async def startup():
        print("🚀 Starting assessment application...")
        await initialize_foundry_assessment_agents_at_startup()
        print("✅ Assessment agent initialization complete, ready to handle requests")
