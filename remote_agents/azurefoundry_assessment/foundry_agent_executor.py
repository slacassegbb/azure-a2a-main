...
class FoundryAssessmentAgentExecutor(AgentExecutor):
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    ...
                    logger.info(
                        "✅ Foundry Assessment agent startup initialization completed successfully"
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Failed to initialize assessment agent at startup: {e}"
                    )
        ...
        async with FoundryAssessmentAgentExecutor._agent_lock:
            if not FoundryAssessmentAgentExecutor._shared_foundry_agent:
                if FoundryAssessmentAgentExecutor._startup_complete:
                    ...
                    raise RuntimeError(
                        "Assessment agent startup initialization failed - agent not available"
                    )

                # Fallback to lazy creation if startup wasn't called
                logger.warning(
                    "⚠️ Assessment agent not initialized at startup, falling back to lazy creation..."
                )
                FoundryAssessmentAgentExecutor._shared_foundry_agent = (
                    FoundryAssessmentAgent()
                )
    ...
    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryAssessmentAgent] = None
    ) -> str:
        if agent is None:
    ...
    async def execute(
        ...
        async with FoundryAssessmentAgentExecutor._request_semaphore:
            ...
            if (
                current_time - FoundryAssessmentAgentExecutor._api_call_window_start
                > 60
            ):
            ...
            if (
                FoundryAssessmentAgentExecutor._api_call_count
                >= FoundryAssessmentAgentExecutor._max_api_calls_per_minute
            ):
                wait_time = 60 - (
                    current_time - FoundryAssessmentAgentExecutor._api_call_window_start
                )
            ...
            time_since_last = (
                current_time - FoundryAssessmentAgentExecutor._last_request_time
            )
            if time_since_last < FoundryAssessmentAgentExecutor._min_request_interval:
                sleep_time = (
                    FoundryAssessmentAgentExecutor._min_request_interval
                    - time_since_last
                )
                logger.warning(
