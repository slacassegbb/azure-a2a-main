"""
AI Foundry Agent Executor for A2A framework - Google Maps Agent.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryGoogleMapsAgent
...
class FoundryAgentExecutor(AgentExecutor):
    _shared_foundry_agent: Optional[FoundryGoogleMapsAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryGoogleMapsAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                try:
                    cls._shared_foundry_agent = FoundryGoogleMapsAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryGoogleMapsAgent:
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundryGoogleMapsAgent()
