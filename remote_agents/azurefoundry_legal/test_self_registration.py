#!/usr/bin/env python3
"""
Test script for Azure Foundry Expert Agent self-registration

This script tests the self-registration functionality for the Azure AI Foundry
agent to ensure it can automatically register with the host agent.
...
DEFAULT_PORT = 8006  # Changed to match legal agent port
...
def create_test_agent_card(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT) -> AgentCard:
    ...
    skills = [
        AgentSkill(
            ...
            description="Provides expert consultation and human-in-the-loop assistance for complex queries",
            tags=['expert', 'consultation', 'human-input'],
            examples=[
                'User requests to speak with a human expert',
                'Complex decision requiring human judgment', 
                'Scenario requiring specialist expertise'
            ],
        ),
        AgentSkill(
            ...
            examples=[
                ...
                'Check availability and schedule optimization'
            ],
        )
    ]

    return AgentCard(
        name='AI Foundry Legal Agent',
        description="An intelligent legal and compliance agent powered by Azure AI Foundry with regulatory expertise.",
...
async def test_self_registration():
    ...
    print("🧪 Testing Azure Foundry Legal Agent Self-Registration")
    print("=" * 60)
...
if __name__ == "__main__":
    print("🚀 Azure Foundry Expert Agent Self-Registration Test")
    print()
