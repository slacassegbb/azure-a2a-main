    ...
    async def execute(
        ...
        async with FoundryLegalAgentExecutor._request_semaphore:
            ...
            if FoundryLegalAgentExecutor._api_call_count >= FoundryLegalAgentExecutor._max_api_calls_per_minute:
                logger.warning(f"⚠️ API call count ({FoundryLegalAgentExecutor._api_call_count}) at limit, requests may be throttled by Azure")
            
            FoundryLegalAgentExecutor._api_call_count += 1
