"""
AI Foundry Agent Executor for A2A framework - Music Agent.
Handles artifact return (audio files via blob storage) following
the same pattern as Word/PowerPoint/Image Generator agents.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryMusicAgent
...
class FoundryAgentExecutor(AgentExecutor):
    _shared_foundry_agent: Optional[FoundryMusicAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryMusicAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                try:
                    cls._shared_foundry_agent = FoundryMusicAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryMusicAgent:
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundryMusicAgent()
    ...
    async def _process_request(self, message_parts, context_id, task_updater, request_context=None):
        try:
            ...
            async for event in agent.run_conversation_stream(session_id, user_message, context_id=context_id):
                if event.startswith("🎵 Calling:") or event.startswith("🎵 Processing:"):
                    tool_desc = event.replace("🎵 Calling: ", "").replace("🎵 Processing: ", "").strip()
                    ...
                    return
                elif event.startswith("Rate limit"):
                    await task_updater.update_status(
                        TaskState.working,
                        message=new_agent_text_message(event, context_id=context_id)
                    )
            ...
            if responses:
                ...
                # Build final completion message with text + artifacts + token usage
