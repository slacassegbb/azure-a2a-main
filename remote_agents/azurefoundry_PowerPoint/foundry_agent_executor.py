"""
AI Foundry Agent Executor for A2A framework - PowerPoint Agent.
Handles artifact (file) delivery via the A2A protocol, mirroring
the pattern used by the image generator agent.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryPowerPointAgent
...
class FoundryAgentExecutor(AgentExecutor):
    _shared_foundry_agent: Optional[FoundryPowerPointAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryPowerPointAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                try:
                    cls._shared_foundry_agent = FoundryPowerPointAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryPowerPointAgent:
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundryPowerPointAgent()
    ...
    async def _process_request(self, message_parts, context_id, task_updater, request_context=None):
        try:
            ...
            # --- Check for file artifacts (same pattern as image generator) ---
                ...
                for artifact in artifacts:
                    if isinstance(artifact, dict) and artifact.get("artifact-uri"):
                        file_with_uri = FileWithUri(
                            name=artifact.get("file-name", "presentation.pptx"),
                            uri=artifact["artifact-uri"],
                            mimeType=artifact.get("mime", "application/vnd.openxmlformats-officedocument.presentationml.presentation"),
                        )
            ...
            if responses:
                ...
                if artifact_parts:
                    final_parts = [Part(root=TextPart(text="Presentation created successfully."))]
                    final_parts.extend(artifact_parts)
                    if hasattr(agent, "last_token_usage") and agent.last_token_usage:
                        final_parts.append(Part(root=DataPart(data={"type": "token_usage", **agent.last_token_usage})))
                    await task_updater.complete(
