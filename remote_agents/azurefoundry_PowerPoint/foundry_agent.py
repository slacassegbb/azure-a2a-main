...
NEVER mention local file paths (e.g. `/tmp/pptx_downloads/...`), sandbox download links
...
Instead, simply state that the presentation has been created and describe its contents.
        ...
        while retry_count <= max_retries:
            try:
                ...
                if text_chunks:
                    ...
                    full_text = re.sub(r'/tmp/pptx_downloads/\S+', '', full_text)
                    full_text = re.sub(r'/tmp/pptx_agent/\S+', '', full_text)
                    ...
                    if self._latest_artifacts:
                        ...
                        file_name = self._latest_artifacts[-1].get("file-name", "presentation.pptx")
                        if blob_uri:
