    ...
    try:
        ...
        async def register_after_startup():
            ...
            if SELF_REGISTRATION_AVAILABLE:
                ...
                try:
                    logger.info(
                        f"🤝 Attempting to register '{agent_card.name}' with host agent at {HOST_AGENT_URL or '[not configured]'}..."
                    )
        ...
        if SELF_REGISTRATION_AVAILABLE and HOST_AGENT_URL:
            ...
            if not HOST_AGENT_URL:
                logger.info(
                    f"📡 '{agent_card.name}' starting without self-registration (A2A_HOST not configured)"
                )
            else:
