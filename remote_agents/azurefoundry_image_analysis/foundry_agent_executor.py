...
class FoundryImageAnalysisAgentExecutor(AgentExecutor):
            ...
            while attempt < 2:
                ...
                try:
                    ...
                    if responses:
                        ...
                        import uuid
                        message_parts = [TextPart(text=final_text_response)]
    ...
    async def execute(
        ...
        # Use semaphore to limit concurrent requests (allows parallel execution up to limit)
        async with FoundryImageAnalysisAgentExecutor._request_semaphore:
            # Track API calls for monitoring (no forced waiting for parallel execution)
            ...
            if FoundryImageAnalysisAgentExecutor._api_call_count >= FoundryImageAnalysisAgentExecutor._max_api_calls_per_minute:
                logger.warning(f"⚠️ API call count ({FoundryImageAnalysisAgentExecutor._api_call_count}) at limit, requests may be throttled by Azure")
            
            FoundryImageAnalysisAgentExecutor._api_call_count += 1
