...
class FoundryImageAnalysisAgentExecutor(AgentExecutor):
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    ...
                    logger.info(
                        "✅ Foundry Image Analysis agent startup initialization completed successfully"
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Failed to initialize image analysis agent at startup: {e}"
                    )
        ...
        async with FoundryImageAnalysisAgentExecutor._agent_lock:
            if not FoundryImageAnalysisAgentExecutor._shared_foundry_agent:
                if FoundryImageAnalysisAgentExecutor._startup_complete:
                    ...
                    raise RuntimeError(
                        "Image analysis agent startup initialization failed - agent not available"
                    )

                # Fallback to lazy creation if startup wasn't called
                logger.warning(
                    "⚠️ Image analysis agent not initialized at startup, falling back to lazy creation..."
                )
                FoundryImageAnalysisAgentExecutor._shared_foundry_agent = (
                    FoundryImageAnalysisAgent()
                )
    ...
    async def _get_or_create_thread(
        self, context_id: str, agent: Optional[FoundryImageAnalysisAgent] = None
    ) -> str:
        if agent is None:
        ...
        try:
            ...
            while attempt < 2:
                ...
                try:
                    ...
                    logger.info(
                        f"🔄 [Image Analysis] Starting streaming for context {context_id}"
                    )
                    async for event in agent.run_conversation_stream(
                        ...
                        logger.debug(
                            f"📥 [Image Analysis] Received event: {event[:100] if isinstance(event, str) else type(event)}"
                        )
                    ...
                    logger.info(
                        f"🎯 [Image Analysis] Completing task for context {context_id} with {len(responses)} responses"
                    )
                    if responses:
                        ...
                        if artifacts:
                            ...
                            artifact_message = new_agent_parts_message(
                                parts=parts, context_id=context_id
                            )
                        ...
                        logger.info(
                            f"📤 Agent response ({len(final_text_response)} chars): {response_preview}"
                        )

                        logger.info(
                            f"✅ [Image Analysis] Marking task as completed for context {context_id}"
                        )
                        await task_updater.complete(
                            message=new_agent_text_message(
                                final_text_response, context_id=context_id
                            )
                        )
                        logger.info(
                            f"✅ [Image Analysis] Task completion sent for context {context_id}"
                        )
                    else:
                        logger.warning("⚠️ No response generated by agent")
                        logger.warning(
                            f"⚠️ [Image Analysis] No responses collected, completing with default message for context {context_id}"
                        )
                        await task_updater.complete(
                            message=new_agent_text_message(
                        ...
                        logger.info(
                            f"✅ [Image Analysis] Task completion sent (no responses) for context {context_id}"
                        )
                    return
    ...
    async def execute(
        ...
        async with FoundryImageAnalysisAgentExecutor._request_semaphore:
            ...
            if (
                current_time - FoundryImageAnalysisAgentExecutor._api_call_window_start
                > 60
            ):
            ...
            if (
                FoundryImageAnalysisAgentExecutor._api_call_count
                >= FoundryImageAnalysisAgentExecutor._max_api_calls_per_minute
            ):
                wait_time = 60 - (
                    current_time
                    - FoundryImageAnalysisAgentExecutor._api_call_window_start
                )
                if wait_time > 0:
                    logger.warning(
            ...
            time_since_last = (
                current_time - FoundryImageAnalysisAgentExecutor._last_request_time
            )
            if (
                time_since_last
                < FoundryImageAnalysisAgentExecutor._min_request_interval
            ):
                sleep_time = (
                    FoundryImageAnalysisAgentExecutor._min_request_interval
                    - time_since_last
