...
from foundry_agent_executor import (
    ...
    initialize_foundry_image_analyzers_at_startup)
...
async def launch_ui(
    ...
    with gr.Blocks(
        theme=gr.themes.Ocean(), title="AI Foundry Image Analysis Agent"
    ) as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
        ...
        **Vision Analysis Capabilities:** Powered by Azure OpenAI GPT-4o vision for comprehensive image understanding:
        ...
        async def _ui_process(message, history):
            if (
                ...
                and hasattr(agent_executor_instance, "_last_received_files")
                and agent_executor_instance._last_received_files
            ):
    ...
    print(
        f"Launching AI Foundry Image Analysis Agent Gradio interface on {host}:{ui_port}..."
    )
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
