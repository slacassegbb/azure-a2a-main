...
def _build_skills():
    ...
    return [
        AgentSkill(
            id='supplier_product_lookup',
            name='Product Lookup',
            description="Search and find products by name, SKU, category, or manufacturer in the supplier catalog.",
            tags=['supplier', 'product', 'search', 'catalog', 'sku'],
            examples=['Find all bond paper products', 'Look up SKU FP-BOND-2024-001', 'What corrugated boxes do you have?'],
        ),
        AgentSkill(
            id='supplier_pricing',
            name='Pricing & Availability',
            description="Check product pricing, bulk discount tiers, stock status, and availability.",
            tags=['supplier', 'pricing', 'availability', 'stock', 'inventory', 'bulk-discount'],
            examples=['What is the price for 500 reams of copy paper?', 'Is product X in stock?', 'Show bulk pricing for stretch film'],
        ),
        AgentSkill(
            id='supplier_lead_time',
            name='Lead Time Query',
            description="Look up estimated delivery and lead times for products and manufacturers.",
            tags=['supplier', 'lead-time', 'delivery', 'shipping', 'eta'],
            examples=['What is the lead time for Domtar bond paper?', 'Which products can ship within 3 days?', 'Compare lead times for packaging tape'],
        ),
        AgentSkill(
            id='supplier_recommendations',
            name='Product Recommendations',
            description="Find alternative or replacement products based on availability, lead time, price, or specifications.",
            tags=['supplier', 'alternative', 'replacement', 'recommendation', 'substitute'],
            examples=['Suggest alternatives for backordered item X', 'Find a cheaper replacement for 3M adhesive tape', 'What can I use instead of product Y?'],
        ),
        AgentSkill(
            id='supplier_update',
            name='Update Supplier Data',
            description="Update product lead times, stock status, or pricing in the supplier database when new information is received.",
            tags=['supplier', 'update', 'lead-time', 'stock-status', 'database'],
            examples=['Update lead time for SKU FP-BOND-006 to 45 days', 'Mark product X as backordered', 'Set new lead time per supplier response'],
        ),
        AgentSkill(
            id='supplier_cross_sell',
            name='Cross-sell & Upsell',
            description="Discover complementary products, accessories, and premium alternatives for cross-selling and upselling.",
            tags=['supplier', 'cross-sell', 'upsell', 'complementary', 'accessory', 'bundle'],
            examples=['What goes well with corrugated boxes?', 'Show me premium alternatives to standard copy paper', 'Suggest accessories for our label printer order'],
        ),
        AgentSkill(
...
def _build_agent_card(host: str, port: int):
    ...
    return AgentCard(
        name='AI Foundry Supplier Agent',
        description="An intelligent supplier agent that provides product catalog lookup, pricing, availability, lead times, and product recommendations from a PostgreSQL database.",
...
def create_a2a_server(host=DEFAULT_HOST, port=DEFAULT_PORT):
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse('AI Foundry Supplier Agent is running!')
...
async def launch_ui(host="0.0.0.0", ui_port=DEFAULT_UI_PORT, a2a_port=DEFAULT_PORT):
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="AI Foundry Supplier Agent") as demo:
        ...
        inp = gr.Textbox(placeholder="Ask about products, pricing, lead times...", show_label=False)
...
def cli(host, port, ui, ui_port):
    """AI Foundry Supplier Agent - A2A server or Gradio UI."""
    if ui:
