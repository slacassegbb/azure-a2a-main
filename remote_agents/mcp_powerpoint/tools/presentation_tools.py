...
def register_presentation_tools(app: FastMCP, presentations: Dict, get_current_presentation_id, get_template_search_directories):
    ...
    def download_presentation(presentation_id: Optional[str] = None, filename: Optional[str] = None) -> Dict:
        """Save a presentation to the server's download directory and return a download URL. The file can then be retrieved via HTTP GET at the returned URL path."""
        pres_id = presentation_id if presentation_id is not None else get_current_presentation_id()
        ...
        try:
            # Save to download directory for HTTP access
            download_dir = "/tmp/pptx_downloads"
            os.makedirs(download_dir, exist_ok=True)
            safe_filename = (filename or f"{pres_id}.pptx").replace("/", "_").replace("\\", "_")
            if not safe_filename.endswith(".pptx"):
                safe_filename += ".pptx"
            download_path = os.path.join(download_dir, safe_filename)
            pres.save(download_path)
