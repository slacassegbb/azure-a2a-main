"""
AI Foundry Agent Executor for A2A framework - GitHub Agent.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryGitHubAgent
...
class FoundryAgentExecutor(AgentExecutor):
    _shared_foundry_agent: Optional[FoundryGitHubAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryGitHubAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                try:
                    cls._shared_foundry_agent = FoundryGitHubAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryGitHubAgent:
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundryGitHubAgent()
