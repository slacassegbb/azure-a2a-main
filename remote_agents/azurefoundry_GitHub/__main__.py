...
def _build_skills():
    ...
    return [
        AgentSkill(
            ...
            description="Create, search, update, and manage GitHub issues. Track bugs, feature requests, and tasks across repositories.",
            tags=['github', 'issues', 'bugs', 'tasks', 'tracking'],
            examples=[
                'Show me all open issues in repo owner/repo',
        ...
        AgentSkill(
            ...
            description="List, review, create, and merge pull requests. Track code reviews and CI status.",
            tags=['github', 'pull-requests', 'code-review', 'merge'],
            examples=[
                'Show me open PRs in owner/repo',
                'Create a PR from feature-branch to main',
                'List reviews on PR #15',
                'Merge PR #20',
            ],
        ),
        AgentSkill(
            ...
            description="Get repository details, list branches, view recent commits, and search code.",
            tags=['github', 'repositories', 'branches', 'commits', 'search'],
            examples=[
                'Get info about owner/repo',
                'List all branches',
                'Show recent commits on main',
                'Search for usages of AuthService in the code',
            ],
        ),
        AgentSkill(
            id='github_project_management',
            name='GitHub Project Management',
            description="Manage labels, milestones, and organize work across repositories.",
            tags=['github', 'labels', 'milestones', 'project-management'],
            examples=[
                'List all labels in the repo',
                'Create a "priority:high" label',
                'Show milestones and their progress',
            ],
        ),
        AgentSkill(
...
def _build_agent_card(host: str, port: int):
    ...
    return AgentCard(
        name='AI Foundry GitHub Agent',
        description="An intelligent GitHub agent for managing issues, pull requests, repositories, and project workflows. Supports search, labels, milestones, and code review operations.",
...
def create_a2a_server(host=DEFAULT_HOST, port=DEFAULT_PORT):
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse('AI Foundry GitHub Agent is running!')
...
async def launch_ui(host="0.0.0.0", ui_port=DEFAULT_UI_PORT, a2a_port=DEFAULT_PORT):
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="AI Foundry GitHub Agent") as demo:
        ...
        inp = gr.Textbox(placeholder="Ask about GitHub issues, PRs, repos...", show_label=False)
...
def cli(host, port, ui, ui_port):
    """AI Foundry GitHub Agent - A2A server or Gradio UI."""
    if ui:
