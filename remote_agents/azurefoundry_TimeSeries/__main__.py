...
def _build_agent_card(host: str, port: int):
    ...
    return AgentCard(
        name='AI Foundry Time Series Agent',
...
def create_a2a_server(host=DEFAULT_HOST, port=DEFAULT_PORT):
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse('AI Foundry Time Series Agent is running!')
...
async def launch_ui(host="0.0.0.0", ui_port=DEFAULT_UI_PORT, a2a_port=DEFAULT_PORT):
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="AI Foundry Time Series Agent") as demo:
...
def cli(host, port, ui, ui_port):
    """AI Foundry Time Series Agent - A2A server or Gradio UI."""
    if ui:
