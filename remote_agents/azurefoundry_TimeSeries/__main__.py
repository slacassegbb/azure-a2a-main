...
def _build_skills():
    ...
    return [
        AgentSkill(
            ...
            description="Predict future values for any time series data using Nixtla TimeGEN-1. Supports single and multi-series with exogenous variables.",
            tags=['timeseries', 'forecast', 'prediction', 'nixtla', 'timegen'],
            examples=[
                'Forecast the next 30 days of stock prices from this data',
                'Predict monthly sales for the next quarter',
                'What will demand look like for the next 12 weeks?',
            ],
        ),
        AgentSkill(
            id='timeseries_anomaly_detection',
            name='Time Series Anomaly Detection',
            description="Detect anomalies, outliers, and unusual patterns in time series data. Use for fraud detection, system monitoring, quality control.",
            tags=['timeseries', 'anomaly', 'detection', 'fraud', 'outlier'],
            examples=[
                'Check this transaction data for anomalies',
                'Find unusual patterns in these server metrics',
                'Are there any outliers in this sales data?',
            ],
        ),
        AgentSkill(
            id='timeseries_historic_forecast',
            name='Historic Forecast (Backtest)',
            description="Backtest predictions on historical data to evaluate model accuracy on known outcomes.",
            tags=['timeseries', 'backtest', 'historic', 'validation'],
            examples=[
                'How accurate would forecasts have been for this data?',
                'Backtest predictions on last quarter sales',
            ],
        ),
        AgentSkill(
            id='timeseries_cross_validation',
            name='Cross-Validation',
            description="Evaluate forecast reliability using rolling-window cross-validation across multiple time periods.",
            tags=['timeseries', 'cross-validation', 'accuracy', 'evaluation'],
            examples=[
                'Evaluate the forecast model accuracy for this dataset',
                'Run cross-validation on this time series',
            ],
        ),
        AgentSkill(
...
def _build_agent_card(host: str, port: int):
    ...
    return AgentCard(
        name='Forecasting and Anomaly Detection Agent',
        description="Time series intelligence agent powered by Nixtla TimeGEN-1. Forecasting, anomaly detection, backtesting, and cross-validation for any time series data.",
...
def create_a2a_server(host=DEFAULT_HOST, port=DEFAULT_PORT):
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse('Forecasting and Anomaly Detection Agent is running!')
...
async def launch_ui(host="0.0.0.0", ui_port=DEFAULT_UI_PORT, a2a_port=DEFAULT_PORT):
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="Forecasting and Anomaly Detection Agent") as demo:
        ...
        inp = gr.Textbox(placeholder="Paste time series data or ask a question...", show_label=False)
...
def cli(host, port, ui, ui_port):
    """Forecasting and Anomaly Detection Agent - A2A server or Gradio UI."""
    if ui:
