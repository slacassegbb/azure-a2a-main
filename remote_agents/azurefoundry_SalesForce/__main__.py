...
async def launch_ui(host: str = "0.0.0.0", ui_port: int = DEFAULT_UI_PORT, a2a_port: int = DEFAULT_PORT):
    ...
    print("Starting Salesforce Agent UI and A2A server...")
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="Salesforce Agent Chat") as demo:
    ...
    print(f"Launching Salesforce Agent Gradio interface on {host}:{ui_port}...")
    demo.queue().launch(server_name=host, server_port=ui_port)
    print("Salesforce Agent Gradio application has been shut down.")
