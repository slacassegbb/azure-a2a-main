...
from foundry_agent import FoundrySalesforceAgent
...
class FoundryAgentExecutor(AgentExecutor):
    ...
    _shared_foundry_agent: Optional[FoundrySalesforceAgent] = None
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundrySalesforceAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        ...
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                ...
                try:
                    cls._shared_foundry_agent = FoundrySalesforceAgent()
    ...
    async def _get_or_create_agent(self) -> FoundrySalesforceAgent:
        """Get the shared Foundry Salesforce agent (with fallback to lazy creation)."""
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundrySalesforceAgent()
    ...
    async def _get_or_create_thread(
        self,
        context_id: str,
        agent: Optional[FoundrySalesforceAgent] = None
    ) -> str:
        if agent is None:
