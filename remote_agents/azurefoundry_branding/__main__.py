...
from foundry_agent_executor import (
    ...
    initialize_foundry_branding_agents_at_startup,
)
...
def run_a2a_server_in_thread(host: str, port: int):
    ...
    print(
        f"Starting AI Foundry Branding & Content Agent A2A server on {host}:{port}..."
    )
...
async def launch_ui(
    ...
    with gr.Blocks(
        theme=gr.themes.Ocean(), title="AI Foundry Branding & Content Agent"
    ) as demo:
        gr.Image(
            "static/a2a.png",
            width=100,
    ...
    print(
        f"Launching AI Foundry Branding & Content Agent Gradio interface on {host}:{ui_port}..."
    )
    demo.queue().launch(
        server_name=host,
        server_port=ui_port,
