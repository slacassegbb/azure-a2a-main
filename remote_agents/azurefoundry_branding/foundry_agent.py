    ...
    async def _setup_file_search(
        ...
        async with FoundryBrandingAgent._file_search_setup_lock:
            ...
            try:
                ...
                FoundryBrandingAgent._shared_vector_store = (
                    project_client.agents.vector_stores.create_and_poll(
                        file_ids=file_ids, name="branding_vectorstore"
                    )
                )
                logger.info(
                    f"Created shared vector store: {FoundryBrandingAgent._shared_vector_store.id}"
                )

                # Create file search tool ONCE
                file_search = FileSearchTool(
                    vector_store_ids=[FoundryBrandingAgent._shared_vector_store.id]
                )
                logger.info(
