"""
AI Foundry Agent Executor for A2A framework - Stock Market Agent.
"""
import asyncio
import logging
import base64
...
from foundry_agent import FoundryStockMarketAgent
...
class FoundryAgentExecutor(AgentExecutor):
    ...
    @classmethod
    async def get_shared_agent(cls) -> Optional[FoundryStockMarketAgent]:
    ...
    async def initialize_at_startup(cls) -> None:
        async with cls._agent_lock:
            if not cls._shared_foundry_agent:
                try:
                    cls._shared_foundry_agent = FoundryStockMarketAgent()
    ...
    async def _get_or_create_agent(self) -> FoundryStockMarketAgent:
        async with FoundryAgentExecutor._agent_lock:
            if not FoundryAgentExecutor._shared_foundry_agent:
                ...
                FoundryAgentExecutor._shared_foundry_agent = FoundryStockMarketAgent()
