...
def _build_skills():
    ...
    return [
        AgentSkill(
            id='stockmarket_price_lookup',
            name='Stock Price Lookup',
            description="Get current and historical stock prices, quotes, and OHLCV data for any publicly traded company.",
            tags=['stockmarket', 'stocks', 'prices', 'quotes', 'ohlcv'],
            examples=[
                'What is the current price of AMD?',
                'Get me the latest AAPL stock quote',
                'Show me TSLA daily prices for the last month',
            ],
        ),
        AgentSkill(
            id='stockmarket_historical_data',
            name='Historical Market Data',
            description="Retrieve historical price data for stocks, forex, crypto, and commodities with daily/weekly/monthly/intraday frequency.",
            tags=['stockmarket', 'historical', 'timeseries', 'forex', 'crypto', 'commodities'],
            examples=[
                'Get 2 years of daily NVDA stock prices',
                'Show me EUR/USD exchange rate history',
                'Get historical Bitcoin prices',
                'What are gold prices over the last year?',
            ],
        ),
        AgentSkill(
            id='stockmarket_technical_analysis',
            name='Technical Analysis',
            description="Calculate technical indicators including SMA, EMA, RSI, MACD, Bollinger Bands, Stochastic, ADX, and 40+ more.",
            tags=['stockmarket', 'technical', 'indicators', 'sma', 'rsi', 'macd'],
            examples=[
                'Calculate the RSI for AAPL',
                'Show me MACD for TSLA',
                'Get Bollinger Bands for MSFT',
            ],
        ),
        AgentSkill(
            id='stockmarket_intelligence',
            name='Market Intelligence',
            description="Access news sentiment analysis, earnings call transcripts, insider transactions, and top market movers.",
            tags=['stockmarket', 'news', 'sentiment', 'earnings', 'insider'],
            examples=[
                'What is the latest news sentiment for AMD?',
                'Show me insider transactions for NVDA',
                'What are today\'s top gainers and losers?',
            ],
        ),
        AgentSkill(
            id='stockmarket_economic_data',
            name='Economic & Commodity Data',
            description="Retrieve economic indicators (GDP, CPI, inflation, unemployment, treasury yields) and commodity prices (oil, gold, wheat, etc.).",
            tags=['stockmarket', 'economic', 'gdp', 'cpi', 'commodities', 'oil', 'gold'],
            examples=[
                'What is the current US inflation rate?',
                'Get historical GDP data',
                'Show me crude oil price history',
...
def create_a2a_server(host=DEFAULT_HOST, port=DEFAULT_PORT):
    ...
    async def health_check(_request: Request) -> PlainTextResponse:
        return PlainTextResponse('AI Foundry Stock Market Agent is running!')
...
async def launch_ui(host="0.0.0.0", ui_port=DEFAULT_UI_PORT, a2a_port=DEFAULT_PORT):
    ...
    with gr.Blocks(theme=gr.themes.Ocean(), title="AI Foundry Stock Market Agent") as demo:
        ...
        inp = gr.Textbox(placeholder="Ask about stocks, crypto, forex, commodities...", show_label=False)
...
def cli(host, port, ui, ui_port):
    """AI Foundry Stock Market Agent - A2A server or Gradio UI."""
    if ui:
