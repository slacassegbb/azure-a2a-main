...
def register_tools():
    ...
    async def insert_numbered_list_near_text(filename: str, target_text: str = None, list_items: list[str] = None, position: str = 'after', target_paragraph_index: int = None, bullet_type: str = 'bullet'):
        ...
        return await content_tools.insert_numbered_list_near_text_tool(filename, target_text, list_items, position, target_paragraph_index, bullet_type)
    ...
    async def add_paragraph(filename: str, text: str, style: str = None,
        ...
        """
        return await content_tools.add_paragraph(filename, text, style, font_name, font_size, bold, italic, color)
    ...
    async def add_heading(filename: str, text: str, level: int = 1,
        ...
        """
        return await content_tools.add_heading(filename, text, level, font_name, font_size, bold, italic, border_bottom)
    ...
    async def set_table_cell_shading(filename: str, table_index: int, row_index: int,
        ...
        return await format_tools.set_table_cell_shading(filename, table_index, row_index, col_index, fill_color, pattern)
    ...
    async def apply_table_alternating_rows(filename: str, table_index: int,
        ...
        return await format_tools.apply_table_alternating_rows(filename, table_index, color1, color2)
    ...
    async def highlight_table_header(filename: str, table_index: int,
        ...
        return await format_tools.highlight_table_header(filename, table_index, header_color, text_color)
    ...
    async def merge_table_cells(filename: str, table_index: int, start_row: int, start_col: int,
        ...
        return await format_tools.merge_table_cells(filename, table_index, start_row, start_col, end_row, end_col)
    ...
    async def merge_table_cells_horizontal(filename: str, table_index: int, row_index: int,
        ...
        return await format_tools.merge_table_cells_horizontal(filename, table_index, row_index, start_col, end_col)
    ...
    async def merge_table_cells_vertical(filename: str, table_index: int, col_index: int,
        ...
        return await format_tools.merge_table_cells_vertical(filename, table_index, col_index, start_row, end_row)
    ...
    async def set_table_cell_alignment(filename: str, table_index: int, row_index: int, col_index: int,
        ...
        return await format_tools.set_table_cell_alignment(filename, table_index, row_index, col_index, horizontal, vertical)
    ...
    async def set_table_alignment_all(filename: str, table_index: int,
        ...
        return await format_tools.set_table_alignment_all(filename, table_index, horizontal, vertical)
    ...
    async def add_footnote_after_text(filename: str, search_text: str, footnote_text: str,
        ...
        return await footnote_tools.add_footnote_after_text(filename, search_text, footnote_text, output_filename)
    ...
    async def add_footnote_before_text(filename: str, search_text: str, footnote_text: str,
        ...
        This enhanced function ensures footnotes display correctly as superscript."""
        return await footnote_tools.add_footnote_before_text(filename, search_text, footnote_text, output_filename)
    ...
    async def add_footnote_enhanced(filename: str, paragraph_index: int, footnote_text: str,
        ...
        return await footnote_tools.add_footnote_enhanced(filename, paragraph_index, footnote_text, output_filename)
    ...
    async def add_footnote_robust(filename: str, search_text: str = None,
        ...
        return await footnote_tools.add_footnote_robust_tool(
    ...
    async def validate_document_footnotes(filename: str):
        ...
        return await footnote_tools.validate_footnotes_tool(filename)
    ...
    async def delete_footnote_robust(filename: str, footnote_id: int = None,
        ...
        return await footnote_tools.delete_footnote_robust_tool(
    ...
    async def replace_paragraph_block_below_header(filename: str, header_text: str, new_paragraphs: list[str], detect_block_end_fn: str = None):
    ...
    async def replace_block_between_manual_anchors(filename: str, start_anchor_text: str, new_paragraphs: list[str], end_anchor_text: str = None, match_fn: str = None, new_paragraph_style: str = None):
        """Replace all content between start_anchor_text and end_anchor_text (or next logical header if not provided)."""
        return await replace_block_between_manual_anchors_tool(filename, start_anchor_text, new_paragraphs, end_anchor_text, match_fn, new_paragraph_style)

    # Download tool (for Azure Container Apps deployment)
    @mcp.tool(
        annotations=ToolAnnotations(
            title="Download Document",
        ),
    )
    async def download_document(filename: str, output_filename: str = None):
        """Save a Word document to the server's download directory and return a download URL.
        The file can then be fetched via HTTP GET at the returned download_url path."""
        ...
        try:
            ...
            safe_name = (output_filename or os.path.basename(filename)).replace("/", "_").replace("\\", "_")
    ...
    async def get_comments_for_paragraph(filename: str, paragraph_index: int):
        ...
        return await comment_tools.get_comments_for_paragraph(filename, paragraph_index)
    ...
    async def set_table_column_width(filename: str, table_index: int, col_index: int,
        ...
        return await format_tools.set_table_column_width(filename, table_index, col_index, width, width_type)
    ...
    async def set_table_column_widths(filename: str, table_index: int, widths: list[float],
                               width_type: str = "points"):
        """Set the widths of multiple table columns."""
        return await format_tools.set_table_column_widths(filename, table_index, widths, width_type)
    ...
    async def set_table_width(filename: str, table_index: int, width: float,
        ...
        return await format_tools.set_table_width(filename, table_index, width, width_type)
    ...
    async def auto_fit_table_columns(filename: str, table_index: int):
        ...
        return await format_tools.auto_fit_table_columns(filename, table_index)
    ...
    async def format_table_cell_text(filename: str, table_index: int, row_index: int, col_index: int,
        ...
        return await format_tools.format_table_cell_text(filename, table_index, row_index, col_index,
    ...
    async def set_table_cell_padding(filename: str, table_index: int, row_index: int, col_index: int,
        ...
        return await format_tools.set_table_cell_padding(filename, table_index, row_index, col_index,
...
def run_server():
    ...
    try:
        if transport_type == 'stdio':
            ...
            # Run with streamable HTTP transport + custom routes for Azure
            import uvicorn
            ...
            async def download_file(request):
                ...
                if not os.path.exists(file_path):
                    return JSONResponse({"error": f"File not found: {safe_fname}"}, status_code=404)
                return FileResponse(
                    file_path,
            ...
            async def health(request):
                return JSONResponse({"status": "ok", "server": "word-mcp-server"})

            # Configure host/port and disable DNS rebinding for Azure Container Apps
            ...
            # Get the Starlette app and inject custom routes
            mcp_starlette = mcp.streamable_http_app()
            mcp_starlette.routes.insert(0, Route("/health", health))
            mcp_starlette.routes.insert(1, Route("/download/{filename:path}", download_file))

            print(f"Server running on streamable-http transport at http://{config['host']}:{config['port']}{config['path']}")
            print(f"Health endpoint: http://{config['host']}:{config['port']}/health")
            print(f"Download endpoint: http://{config['host']}:{config['port']}/download/{{filename}}")
            uvicorn.run(mcp_starlette, host=config['host'], port=config['port'])
