...
class FoundryTemplateAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        try:
            ...
            # Get any artifacts (videos) that were generated
            ...
            if artifacts:
                ...
                for artifact in artifacts:
                    if isinstance(artifact, dict) and artifact.get("artifact-uri"):
                        # For artifacts with URIs (like videos), create FilePart with FileWithUri
                        file_with_uri = FileWithUri(
                            name=artifact.get("file-name", "video.mp4"),
                            uri=artifact["artifact-uri"],
                            mimeType=artifact.get("mime", "video/mp4")
                        )
                ...
                # Send artifacts as a separate message
                if artifact_parts:
                    ...
                    logger.info(f"📤 Sent {len(artifact_parts)} video artifact(s)")
            
            # Emit the final response with both text AND artifacts
            if responses:
                ...
                logger.info(f"📤 Agent response ({len(final_response)} chars, {len(artifact_parts)} artifacts): {response_preview}")
                ...
                # No text responses, but we may have video artifacts
                logger.warning("⚠️ No text response generated by agent")
                
                if artifact_parts:
                    # We have video artifacts but no text - complete with artifacts and default text
                    logger.info(f"✅ Completing with {len(artifact_parts)} video artifacts and default text")
                    final_parts = [Part(root=TextPart(text="Video generated successfully."))]
                    final_parts.extend(artifact_parts)
