...
async def register_with_host_agent(
    ...
    if not host_url:
        logger.info("Host agent URL not provided; skipping self-registration")
        return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("success"):
                        ...
                        logger.warning(f"Host agent rejected registration: {result.get('error')}")
                        if attempt == max_retries - 1:
                            return False
                        continue
                else:
                    logger.warning(f"Host agent returned status {response.status_code}: {response.text}")
                    if attempt == max_retries - 1:
                        return False
                    continue
            ...
            if attempt == 0:
                logger.info(f"Host agent not available at {registration_url}")
            if attempt == max_retries - 1:
                logger.info(f"Host agent still not available after {max_retries} attempts - continuing without registration")
                return False
            continue
        except Exception as e:
            logger.warning(f"Self-registration attempt {attempt + 1} failed: {e}")
            if attempt == max_retries - 1:
                return False
            continue
