...
log_debug(f"Environment loaded, A2A_HOST: {settings.server.a2a_host}")
log_debug(f"WebSocket URL: {settings.server.websocket_server_url}")
log_debug(f"Azure Tenant ID: {settings.auth.azure_tenant_id}")
...
from service.websocket_server import (set_auth_service, start_websocket_server,
                                      stop_websocket_server)
from service.websocket_streamer import (cleanup_websocket_streamer,
                                        get_websocket_streamer)

# Authentication constants from centralized settings
...
def main():
    ...
    async def health_check():
        ...
        return {
            ...
            "client_id": settings.auth.azure_client_id or "not_set",
        }
...
"""
    ...
    def upload_to_azure_blob(
        ...
        try:
            ...
            # Get Azure connection details from settings
            ...
            if storage_account_name and not connection_string:
                ...
                print(
                    f"[WARN] No Azure Storage configuration found, returning local path"
                )
            ...
            print(
                f"[INFO] File uploaded to Azure Blob (managed identity): {blob_client.url}"
            )
    ...
    async def upload_voice(file: UploadFile = File(...)):
        ...
        try:
            ...
            try:
                ...
                from hosts.multiagent.a2a_document_processor import \
                    process_audio
    ...
    # Setup the connection details from centralized settings
    host = settings.server.ui_host
