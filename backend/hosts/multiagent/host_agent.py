...
from a2a.types import (AgentCard, DataPart, Message, MessageSendConfiguration,
...
"""
    ...
    async def send_message(
        ...
        print(
            f"[DEBUG] send_message called for agent: {agent_name}, message: {message}"
        )
        print(
            f"[DEBUG] tool_context.state before send: {json.dumps(tool_context.state, default=str)}"
        )
        print(
            f"[DEBUG] MessageSendParams (as dict): {json.dumps(request.model_dump(), default=str)}"
        )
...
async def convert_part(part: Part, tool_context: ToolContext):
    ...
    if part.root.kind == "text":
        ...
        if isinstance(artifact_response, DataPart) and getattr(
            artifact_response, "data", None
        ):
            print(
                f"[DEBUG] convert_part: received artifact response with data {artifact_response.data}"
            )
        tool_context.actions.skip_summarization = True
        tool_context.actions.escalate = True
