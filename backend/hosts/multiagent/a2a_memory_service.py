...
from log_config import (log_error, log_info, log_memory_debug, log_success,
                        log_warning)

# Load configuration from centralized settings
service_endpoint = settings.azure_search.service_endpoint
admin_key = settings.azure_search.admin_key
vector_dimension = settings.azure_search.vector_dimension
index_name = settings.azure_search.index_name
vector_profile_name = settings.azure_search.vector_profile_name
vector_algorithm_name = settings.azure_search.vector_algorithm_name

# Azure OpenAI configuration
azure_openai_endpoint = settings.azure_openai_embeddings.endpoint
azure_openai_key = settings.azure_openai_embeddings.key
azure_openai_deployment = settings.azure_openai_embeddings.deployment
...
class A2AMemoryService:
    def __init__(self):
        ...
        if not admin_key or not service_endpoint:
            print(
                f"⚠️ Azure Search not configured (admin_key: {admin_key is not None}, service_endpoint: {service_endpoint is not None})"
            )
        ...
        try:
            ...
            self.index_client = SearchIndexClient(
                endpoint=service_endpoint, credential=self.credential
            )
    ...
    def _ensure_index_exists(self) -> bool:
        ...
        try:
            ...
            if self._is_index_compatible(existing_index):
                ...
                log_memory_debug(
                    f"Existing index configuration does not match expected settings. Recreating index {self.index_name}."
                )
    ...
    def _create_interactions_index(self) -> bool:
        ...
        fields = [
            ...
            SimpleField(
                name="processing_time_seconds", type=SearchFieldDataType.Double
            ),
        ...
        # Create index
        index = SearchIndex(
            name=self.index_name, fields=fields, vector_search=vector_search_config
        )
    ...
    def _wait_for_index(
        self, index_name: str, max_retries: int = 5, delay: int = 5
    ) -> bool:
    ...
    async def _create_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            response = self.openai_client.embeddings.create(
                model=azure_openai_deployment, input=text
            )
    ...
    async def store_interaction(self, interaction_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if len(embedding) != vector_dimension:
                ...
                log_memory_debug(
                    "To resolve, set AZURE_SEARCH_VECTOR_DIMENSION to the embedding size and restart the service."
                )
                return False
            ...
            document = {
                ...
                "processing_time_seconds": interaction_data.get(
                ...
                "outbound_payload": json.dumps(
                    interaction_data.get("outbound_payload", {})
                ),
                "inbound_payload": json.dumps(
                    interaction_data.get("inbound_payload", {})
                ),
    ...
    def clear_all_interactions(self) -> bool:
        ...
        if not self._enabled:
            log_memory_debug(
                "Memory service disabled - skipping clear_all_interactions"
            )
            return True  # Return success to avoid breaking the flow
        ...
        try:
            ...
            upload_result = self.search_client.upload_documents(
                documents=delete_actions
            )

            # Check results
            success_count = sum(1 for result in upload_result if result.succeeded)
            log_success(
                f"Successfully deleted {success_count}/{len(doc_ids)} documents"
            )
