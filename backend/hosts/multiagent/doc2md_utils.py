...
def _load_gpt_configuration() -> dict:
    ...
    missing = []
    if not settings.azure_ai_foundry.openai_gpt_api_base:
        missing.append("AZURE_OPENAI_GPT_API_BASE")
    if not settings.azure_ai_foundry.openai_gpt_api_key:
        missing.append("AZURE_OPENAI_GPT_API_KEY")
    if not settings.azure_ai_foundry.openai_gpt_api_version:
        missing.append("AZURE_OPENAI_GPT_API_VERSION")
    if not settings.azure_ai_foundry.openai_gpt_deployment:
        missing.append("AZURE_OPENAI_GPT_DEPLOYMENT")
    ...
    config = {
        "AZURE_OPENAI_GPT_API_BASE": settings.azure_ai_foundry.openai_gpt_api_base,
        "AZURE_OPENAI_GPT_API_KEY": settings.azure_ai_foundry.openai_gpt_api_key,
        "AZURE_OPENAI_GPT_API_VERSION": settings.azure_ai_foundry.openai_gpt_api_version,
        "AZURE_OPENAI_GPT_DEPLOYMENT": settings.azure_ai_foundry.openai_gpt_deployment,
    }
    ...
    if file_suffix in supported_conversion_types:
        ...
        if file_suffix == ".pdf":
            ...
            command = [
                ...
                "--convert-to",
                "pdf",  # Specify conversion format
                "--outdir",
                os.path.dirname(output_file),  # Output directory
            ...
            try:
                ...
                print(
                    "LibreOffice not found. Please ensure LibreOffice is installed and the path is correct."
                )
                return ""
    else:
...
def process_image(file, markdown_out_dir):
    if ".png" in file:
        ...
        markdown_file_out = os.path.join(
            markdown_out_dir, os.path.basename(file).replace(".png", ".txt")
        )
