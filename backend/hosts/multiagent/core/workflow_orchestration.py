...
- When the user provides information in a follow-up, create a NEW task with that information"""
        ...
        while plan.goal_status == "incomplete" and iteration < max_iterations:
            ...
            # =========================================================
            # QUEUE DEPTH: Notify user if messages are queued behind
            # =========================================================
            if hasattr(self, '_get_queue_depth') and self._get_queue_depth:
                queue_depth = self._get_queue_depth(context_id)
                if queue_depth > 0:
                    await self._emit_granular_agent_event(
                        "foundry-host-agent",
                        f"{queue_depth} message(s) queued — will process after current workflow completes",
                        context_id,
                        event_type="info",
...
Analyze the plan and determine the next step."""
            ...
            try:
                ...
                if next_step is None:
                    raise Exception("Planner failed to produce a response after retries")
