...
def get_content_understanding_client():
    ...
    if not azure_ai_service_endpoint:
        print(
            "⚠️ Missing AZURE_CONTENT_UNDERSTANDING_ENDPOINT (or AZURE_AI_SERVICE_ENDPOINT) environment variable."
        )
        return None

    azure_ai_service_endpoint = azure_ai_service_endpoint.strip().rstrip("/")

    azure_ai_service_api_version = settings.azure_content_understanding.api_version
...
def process_audio(audio_path: str, return_text: bool = False):
    ...
    try:
        ...
        print(
            f"[DEBUG] Analysis result keys: {list(result.keys()) if isinstance(result, dict) else 'Not a dict'}"
        )
...
def process_video(video_path):
    ...
    try:
        ...
        template_path = os.path.join(
            module_dir, "analyzer_templates", "video_content_understanding.json"
        )
        ...
        for i, content in enumerate(video_content):
            ...
            time_range = (
                f"{content['startTimeMs']/1000:.1f}s-{content['endTimeMs']/1000:.1f}s"
            )
...
def process_document(document_path):
    ...
    partial_process_image = partial(
        doc2md_utils.process_image, markdown_out_dir=markdown_out_dir
    )
    ...
    try:
        ...
        if file_bytes is None and artifact_info:
            ...
            if "file_bytes" in artifact_info:
                ...
                print(
                    f"[A2ADocumentProcessor] Using file bytes from artifact_info: {len(file_bytes)} bytes"
                )
                ...
                if "blob.core.windows.net" in artifact_uri:
                    print(
                        f"[A2ADocumentProcessor] Downloading file from Azure Blob: {artifact_uri}"
                    )
                    ...
                    try:
                        ...
                        print(
                            f"[A2ADocumentProcessor] Downloaded {len(file_bytes)} bytes from Azure Blob"
                        )
                    except Exception as e:
                        print(
                            f"[A2ADocumentProcessor] Error downloading from Azure Blob: {e}"
                        )
                        return {
                            "success": False,
                            "error": f"Failed to download from Azure Blob: {e}",
                        }
                else:
                    print(
                        f"[A2ADocumentProcessor] Unknown URI type (not Azure Blob): {artifact_uri}"
                    )
                    return {
                        "success": False,
                        "error": "Unknown URI type (not Azure Blob)",
                    }

        if file_bytes is None:
            print(
                f"[A2ADocumentProcessor] Could not find file bytes in file part or artifact URI"
            )
        ...
        interaction_data = {
            ...
            "task_id": artifact_info.get("task_id")
            if artifact_info
            else str(uuid.uuid4()),
        ...
        print(
            f"[A2ADocumentProcessor] Content length: {len(processed_content)} characters"
        )
