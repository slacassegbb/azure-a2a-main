...
from state.state import StateConversation, StateEvent, StateMessage, StateTask
...
def get_task_id(obj: Any, default: str = None) -> str:
    ...
    try:
        ...
        # Use getattr as final fallback
        return getattr(
            obj, "taskId", getattr(obj, "task_id", getattr(obj, "id", default or ""))
        )
    except Exception:
        return default or ""
...
class WebSocketStreamer:
    ...
    def __init__(self, websocket_url: str | None = None):
        ...
        websocket_url = websocket_url or settings.server.websocket_server_url
        ...
        self._emitted_file_uris: Dict[
            str, Set[str]
        ] = {}  # {conversation_id: {file_uri, ...}}
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        log_debug(
                            f"WebSocket streamer connected to {self.websocket_url}"
                        )
                        return True
                    else:
                        logger.warning(
                            f"WebSocket server health check failed: {response.status_code}"
                        )

                except httpx.ConnectError:
                    if attempt < 2:  # Don't log error on last attempt
                        logger.info(
                            f"WebSocket server not ready, attempt {attempt + 1}/3..."
                        )
            ...
            logger.error(
                f"❌ Failed to connect to WebSocket server at {self.websocket_url}"
            )
    ...
    async def _send_event(
        self, event_type: str, data: Dict[str, Any], partition_key: Optional[str] = None
    ) -> bool:
        ...
        if not self.is_initialized or not self.http_client:
            logger.error(
                f"WebSocket streamer not initialized, cannot send {event_type} event"
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log_debug(
                    f"✅ Event {event_type} sent successfully to {client_count} WebSocket clients"
                )
                logger.info(
                    f"✅ Event {event_type} sent successfully to {client_count} WebSocket clients"
                )
                ...
                logger.error(
                    f"❌ Failed to send {event_type} event: HTTP {response.status_code}"
                )
                log_debug(
                    f"❌ Failed to send {event_type} event: HTTP {response.status_code}"
                )
    ...
    async def stream_message_received(
    ...
    async def stream_conversation_created(
        self, conversation: StateConversation
    ) -> bool:
    ...
    async def stream_conversation_updated(
        self, conversation: StateConversation
    ) -> bool:
    ...
    async def stream_file_uploaded(
        self, file_info: Dict[str, Any], conversation_id: str
    ) -> bool:
        ...
        if file_uri in self._emitted_file_uris[conversation_id]:
            logger.debug(
                f"Skipping duplicate file_uploaded event for URI in conversation {conversation_id[:8]}...: {file_uri[:80]}..."
            )
    ...
    async def stream_form_submitted(
        self, form_data: Dict[str, Any], conversation_id: str
    ) -> bool:
    ...
    async def stream_agent_registered(
        self, agent_path: str, agent_name: Optional[str] = None
    ) -> bool:
        ...
        data = {
            ...
            "status": "registered",
            "timestamp": datetime.now().isoformat(),
            "avatar": f"/api/agents/{agent_name}/avatar"
            if agent_name
            ...
            if hasattr(message, "parts") and message.parts:
                ...
                for part in message.parts:
                    if hasattr(part, "text") and part.text:
                        content.append({"type": "text", "content": part.text})
...
async def get_websocket_streamer() -> Optional[WebSocketStreamer]:
    ...
    if _websocket_streamer is None:
        ...
        # Get WebSocket server URL from centralized settings
        websocket_url = settings.server.websocket_server_url
