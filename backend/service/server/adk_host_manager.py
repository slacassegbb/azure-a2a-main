...
def get_task_id(obj: Any, default: str = None) -> str:
    ...
    try:
        ...
        # Final fallback using getattr
        return getattr(
            obj, "taskId", getattr(obj, "task_id", default or str(uuid.uuid4()))
        )
    except Exception:
        return default or str(uuid.uuid4())
...
# Ensure backend root (containing hosts package) is importable when running standalone
...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(
        ...
        self.api_key = api_key or settings.google_adk.api_key
        self.uses_vertex_ai = uses_vertex_ai or settings.google_adk.use_vertexai
    ...
    def _initialize_host(self):
        # Defer agent creation to avoid blocking on startup
    ...
    def sanitize_message(self, message: Message) -> Message:
        if get_context_id(message):
            conversation = self.get_conversation(get_context_id(message))
            if not conversation:
                return message
            # Check if the last event in the conversation was tied to a task.
            if conversation.messages:
                task_id = get_task_id(conversation.messages[-1])
    ...
    async def process_message(self, message: Message):
        ...
        # Add message ID to pending list
                ...
                async def stream_conversation_created():
                    try:
                        ...
                        if streamer:
                            ...
                            if success:
                                print(
                                    f"[DEBUG] Conversation creation streamed: {event_data}"
                                )
                            else:
                                print("[DEBUG] Failed to stream conversation creation")
                        else:
                            print(
                                "[DEBUG] WebSocket streamer not available for conversation creation"
                            )
                    except Exception as e:
    ...
    def register_agent(self, url):
        ...
        # Reset agent initialization to force re-creation with new agents
        self._agent_initialized = False
        self._host_runner = None
