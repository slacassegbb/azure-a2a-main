...
from a2a.types import (AgentCard, DataPart, FilePart, FileWithBytes,
                       FileWithUri, Message, Task, TaskState, TaskStatus,
                       TextPart)
from hosts.multiagent.foundry_agent_a2a import FoundryHostAgent2
from log_config import log_debug, log_error, log_info, log_success
...
class FoundryHostManager(ApplicationManager):
    def __init__(self, http_client: httpx.AsyncClient, *args, **kwargs):
        ...
        try:
            ...
            self._host_agent = FoundryHostAgent2(
                [], self._http_client, create_agent_at_startup=True
            )
    ...
    def foundry_content_to_message(self, resp, context_id, task_id=None):
        ...
        # Handle single dict with artifact metadata (from DataPart)
            ...
            "artifact-uri" in resp or "artifact-id" in resp
        ):
        ...
        # Handle Message object
            ...
            log_debug(
                f"Processing as Message object with {len(resp.parts) if resp.parts else 0} parts"
            )
            for part in resp.parts:
                ...
                if kind == "text":
                    ...
                    log_debug(
                        f"Unknown part kind: {kind}, content: {str(root)[:200]}..."
                    )
    ...
    async def process_message(
        ...
        log_debug(
            f"process_message: Agent Mode = {agent_mode}, Inter-Agent Memory = {enable_inter_agent_memory}, Workflow = {workflow[:50] if workflow else None}"
        )
            ...
            try:
                ...
                if streamer:
                    ...
                    success = await streamer._send_event(
                        "conversation_created", event_data, context_id
                    )
                    if success:
                    ...
                    log_debug(
                        "WebSocket streamer not available for conversation creation"
                    )
        ...
        try:
            ...
            if streamer:
                ...
                success = await streamer._send_event(
                    "task_created", event_data, context_id
                )
                if success:
        ...
        def event_logger(event_dict):
            ...
            if "args" in event_dict:
                ...
                if event_dict["name"] == "send_message" and isinstance(
                    event_dict["args"], dict
                ):
            ...
            if "args" in event_dict:
                log_debug(
                    f"Streaming tool call event to WebSocket for agent: {agent_name}"
                )
                try:
                    import asyncio
                    ...
                    async def stream_tool_call():
                        try:
                            ...
                            if streamer:
                                ...
                                success = await streamer._send_event(
                                    "tool_call", event_data, context_id
                                )
                                if success:
                                ...
                                log_debug(
                                    "WebSocket streamer not available for tool call"
                                )
                        except Exception as e:
            ...
            # Stream tool response events to WebSocket for granular visibility
                ...
                log_debug(
                    f"Streaming tool response event to WebSocket for agent: {agent_name}"
                )
                try:
                    import asyncio
                        ...
                        try:
                            ...
                            if streamer:
                                ...
                                if isinstance(output, dict) and "error" in output:
                                    ...
                                    "error" in str(output).lower()
                                    or "failed" in str(output).lower()
                                ):
                                    status = "failed"
                                ...
                                success = await streamer._send_event(
                                    "tool_response", event_data, context_id
                                )
                                if success:
                                    log_debug(
                                        f"Tool response event streamed: {event_data}"
                                    )
                                else:
                                    log_debug("Failed to stream tool response event")
                            else:
                                log_debug(
                                    "WebSocket streamer not available for tool response"
                                )
                        except Exception as e:
                            log_debug(
                                f"Error streaming tool response to WebSocket: {e}"
                            )
                            # Don't let WebSocket errors break the main flow
                            pass
        ...
        user_text = (
            message.parts[0].root.text
            if message.parts and message.parts[0].root.kind == "text"
            else ""
        )
        log_debug(
            f"About to call run_conversation_with_parts with message parts: {len(message.parts)} parts, agent_mode: {agent_mode}, enable_inter_agent_memory: {enable_inter_agent_memory}, workflow: {workflow[:50] if workflow else None}"
        )
        responses = await self._host_agent.run_conversation_with_parts(
            message.parts,
            context_id,
        ...
        log_debug(
            f"FoundryHostAgent responses count: {len(responses) if responses else 'None'}"
        )
        log_debug(f"FoundryHostAgent responses: {responses}")

        if not responses:
            log_debug(
                "WARNING: No responses from FoundryHostAgent - this will cause no messages to be sent to frontend!"
            )
        ...
        for tool_event in tool_call_events:
            ...
            if "TOOL CALL: send_message" in tool_event.content.parts[0].root.text:
                ...
                m = re.search(
                    r"'agent_name': '([^']+)'", tool_event.content.parts[0].root.text
                )
                if m:
        ...
        log_debug(
            f"Agent names from tool calls (for debug only): {agent_names_from_tools}"
        )
        ...
        async def stream_task_status_update(target_agent: str, target_state: TaskState):
            try:
                ...
                success = await streamer._send_event(
                    "task_updated", event_data, context_id
                )
                if success:
        ...
        for resp_index, resp in enumerate(responses):
            log_debug(
                f"Response {resp_index}: type={type(resp)}, is_dict={isinstance(resp, dict)}"
            )
            if isinstance(resp, dict):
                log_debug(f"Response {resp_index} dict keys: {list(resp.keys())}")
                log_debug(
                    f"Response {resp_index} has artifact-uri: {'artifact-uri' in resp}"
                )
            print("[DEBUG] Response parts:", getattr(resp, "parts", None))
            msg = self.foundry_content_to_message(resp, context_id, task_id)
            log_debug(
                f"Message created with {len(msg.parts) if hasattr(msg, 'parts') else 0} parts"
            )
            if conversation:
            ...
            log_debug(
                f"Using correct foundry host agent name for response {resp_index}: {actor_name}"
            )
            log_debug(
                f"Prevented incorrect attribution to individual agents: {agent_names_from_tools}"
            )
            ...
            try:
                ...
                if streamer:
                    ...
                    log_debug(
                        f"WebSocket streaming - resp type: {type(resp)}, msg has parts: {hasattr(msg, 'parts')}"
                    )
                    ...
                    if isinstance(resp, str):
                        ...
                        for part in msg.parts:
                            log_debug(
                                f"Processing part: {type(part)}, has root: {hasattr(part, 'root')}"
                            )
                            root = part.root
                            if isinstance(root, TextPart):
                                text_parts.append(root.text)
                            elif isinstance(root, DataPart) and isinstance(
                                root.data, dict
                            ):
                                artifact_uri = root.data.get("artifact-uri")
                                log_debug(
                                    f"Found DataPart with dict, has artifact-uri: {bool(artifact_uri)}"
                                )
                                if artifact_uri:
                                    log_debug(
                                        f"Adding image to content: {root.data.get('file-name')}"
                                    )
                                    image_parts.append(
                                        {
                                            "type": "image",
                                            ...
                                            "mediaType": root.data.get(
                                                "media-type", "image/png"
                                            ),
                                            "storageType": root.data.get(
                        ...
                        if image_parts:
                            pending_list = self._pending_artifacts.setdefault(
                                context_id, []
                            )
                    ...
                    if not any(
                        item.get("type") == "image" for item in event_data["content"]
                    ):
                    ...
                    for content_item in event_data["content"]:
                        if content_item.get("type") == "image":
                            log_debug(
                                f"Found file content with uri: {content_item.get('uri')}"
                            )
                        if content_item.get("type") == "image" and content_item.get(
                            "uri"
                        ):
                            ...
                            log_debug(
                                f"File uploaded event sent for agent artifact: {file_info['filename']}"
                            )
            ...
            if hasattr(resp, "status") and hasattr(resp.status, "state"):
                ...
                "input required" in resp_lower
                or "please provide" in resp_lower
                or "waiting for" in resp_lower
            ):
            ...
            if task.status.message and (
                not conversation.messages
                or conversation.messages[-1] != task.status.message
            ):
        ...
        remaining_agents = [
            agent_name
            for agent_name in agent_names_from_tools
            if agent_name not in completed_agents
        ]
        for leftover_agent in remaining_agents:
            log_debug(
                f"Emitting completion status for agent without direct response: {leftover_agent}"
            )
            try:
                asyncio.create_task(
                    stream_task_status_update(leftover_agent, TaskState.completed)
                )
            except Exception as e:
                log_debug(
                    f"Error scheduling fallback status update for {leftover_agent}: {e}"
                )
    ...
    def add_event(self, event: Event):
        ...
        event.timestamp = (
            event.timestamp or __import__("datetime").datetime.utcnow().timestamp()
        )
    ...
    async def handle_self_registration(
        ...
        try:
            log_debug(
                f"🤝 Host manager handling self-registration from: {agent_address}"
            )
            ...
            success = await self._host_agent.register_remote_agent(
            ...
            if success:
                ...
                existing_index = next(
                    (i for i, a in enumerate(self._agents) if a.url == agent_address),
                    None,
                )
                ...
                if existing_index is not None:
                    ...
                    log_debug(
                        f"🔄 Updated {agent_card.name} in UI agent list (was: {old_name})"
                    )
                else:
                    # Add new agent
                    self._agents.append(agent_card)
                ...
                try:
                    ...
                    if websocket_server:
                        ...
                        log_debug(
                            f"🔔 Triggered immediate agent registry sync for {agent_card.name}"
                        )
                    else:
                        log_debug(
                            f"⚠️ WebSocket server not available for immediate sync"
                        )
            ...
            if success:
                ...
                try:
                    ...
                    if websocket_server:
                        websocket_server.trigger_immediate_sync()
                        log_debug(
                            f"🔔 Triggered immediate agent registry sync after removing {agent_name}"
                        )
                    else:
                        log_debug(
                            f"⚠️ WebSocket server not available for immediate sync"
