...
class ConversationServer:
    ...
    def __init__(self, app: FastAPI, http_client: httpx.AsyncClient):
        agent_manager = settings.server.a2a_host
        self.manager: ApplicationManager

        # Get configuration from centralized settings
        api_key = settings.google_adk.api_key
        uses_vertex_ai = settings.google_adk.use_vertexai
            ...
            for part in message_data["parts"]:
                if "root" in part:
                    ...
                    if root.get("kind") == "text":
                        ...
                        from a2a.types import FilePart as A2AFilePart
                        from a2a.types import Part as A2APart
                        ...
                        if file_data.get("role"):
                            file_part_kwargs["metadata"] = {
                                "role": file_data.get("role")
                            }
                            print(
                                f"🎭 [server.py] Setting metadata role='{file_data.get('role')}' for file: {file_data.get('name', 'unknown')}"
                            )

                        transformed_parts.append(
                            A2APart(root=A2AFilePart(**file_part_kwargs))
                        )
                    else:
