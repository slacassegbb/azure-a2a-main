...
class AsyncTaskQueue:
    ...
    def __init__(self, max_workers: int = 5, queue_size: int = 10000):
    ...
    async def enqueue(
        ...
        logger.debug(
            f"[AsyncQueue] Stored task metadata, total tracked: {len(self.task_metadata)}"
        )
        ...
        try:
            ...
            await asyncio.wait_for(
                self.task_queue.put(task), timeout=5.0  # Max 5s wait to enqueue
            )
    ...
    async def _worker_loop(self, worker_id: int):
        ...
        while not self._shutdown:
            ...
            try:
                ...
                logger.debug(
                    f"[AsyncQueue] Worker {worker_id} waiting for next task..."
                )
                ...
                logger.info(
                    f"[AsyncQueue] 📥 Worker {worker_id} dequeued task {task.task_id}"
                )
                ...
                if task.is_expired():
                    ...
                    await self._handle_failure(
                        task, "Task expired before processing", retry=False
                    )
                ...
                logger.info(
                    f"[AsyncQueue] ⚙️ Worker {worker_id} processing task {task.task_id}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"[AsyncQueue] 🔄 Calling processor function for task {task.task_id}..."
                    )
                    result = await asyncio.wait_for(
                        self.processor_func(task), timeout=task.timeout_seconds
                    )
                    ...
                    if self.on_result:
                        logger.info(
                            f"[AsyncQueue] 📡 Invoking result callback for task {task.task_id}"
                        )
                        try:
                            await self.on_result(task, result)
                            logger.info(
                                f"[AsyncQueue] ✅ Result callback completed for task {task.task_id}"
                            )
                        except Exception as cb_err:
                            logger.error(
                                f"[AsyncQueue] ❌ Result callback error: {type(cb_err).__name__}: {cb_err}"
                            )
                    else:
                        logger.warning(
                            f"[AsyncQueue] ⚠️ No result callback registered!"
                        )
    ...
    async def _handle_failure(self, task: AsyncTask, error: str, retry: bool = True):
        """
