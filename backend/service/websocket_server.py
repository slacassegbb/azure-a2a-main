...
from log_config import log_error, log_info, log_warning, log_websocket_debug
...
class WebSocketManager:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                response = requests.get(
                    f"http://{self.backend_host}:{self.backend_port}/agents", timeout=20
                )
                ...
                if response.status_code == 200:
                    ...
                    for agent_data in agents_data.get("agents", []):
                        agent_info = {
                            ...
                            "defaultInputModes": agent_data.get(
                                "defaultInputModes", []
                            ),
                            "defaultOutputModes": agent_data.get(
                                "defaultOutputModes", []
                            ),
                            "status": agent_data.get(
                                "status", "unknown"
                            ),  # Use actual status from backend
                            "avatar": agent_data.get("iconUrl")
                            or "/placeholder.svg?height=32&width=32",
                ...
                log_websocket_debug(
                    f"Error getting agents from backend (attempt {attempt + 1}): {e}"
                )
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"Could not get agent registry from backend after {max_retries + 1} attempts: {e}"
                    )

        # Fallback to empty list if backend not available after all retries
        log_websocket_debug(
            "Returning empty agent list after all retry attempts failed"
        )
        return []
    ...
    async def connect(self, websocket: WebSocket, token: Optional[str] = None):
        ...
        if token and auth_service:
            ...
            if user_data:
                ...
                if auth_service:
                    ...
                    logger.warning(
                        "Auth service not available - cannot track active user"
                    )
                logger.info(
                    f"Authenticated WebSocket connection for user: {auth_conn.username} ({auth_conn.email})"
                )
            else:
        ...
        try:
            ...
            if agents:
                ...
                logger.info(
                    f"Sent agent registry with {len(agents)} agents to new client"
                )
        except Exception as e:
        ...
        logger.info(
            f"WebSocket client connected. Total: {total_connections}, Authenticated: {authenticated_connections}"
        )
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        authenticated_connections = len(self.authenticated_connections)
        logger.info(
            f"WebSocket client disconnected. Total: {total_connections}, Authenticated: {authenticated_connections}"
        )
    ...
    async def broadcast_user_list_update(self):
        ...
        try:
            if auth_service:
                ...
                logger.info(
                    f"Broadcasted user list update: {len(active_users)} active users"
                )
        except Exception as e:
    ...
    async def emit_agent_status_update(self, status_event: Dict[str, Any]):
        ...
        try:
            ...
            await self.broadcast_event(event_data)
            print(
                f"[WEBSOCKET] Emitted agent status update: {status_event.get('agent_name')} -> {status_event.get('status')}"
            )
...
def create_websocket_app() -> FastAPI:
    ...
    async def handle_websocket_message(websocket: WebSocket, message: Dict[str, Any]):
        ...
        if message_type == "chat":
            ...
            if auth_conn:
                ...
                logger.info(
                    f"Chat message from {auth_conn.username}: {message.get('text', '')[:50]}..."
                )
            else:
            ...
            logger.info(
                f"Shared message broadcasted: {message_data.get('content', '')[:50]}..."
            )
            ...
            logger.info(
                f"Shared inference started broadcasted for conversation: {inference_data.get('conversationId')}"
            )
            ...
            await websocket_manager.broadcast_event(inference_event)
            logger.info(
                f"Shared inference ended broadcasted for conversation: {inference_data.get('conversationId')}"
            )
    ...
    async def refresh_agents():
        ...
        try:
            ...
            return JSONResponse(
                {"success": True, "message": "Agent registry refresh triggered"}
            )
        except Exception as e:
    ...
    async def get_agents():
        ...
        try:
            ...
            return JSONResponse(
                {"success": True, "agents": agents, "count": len(agents)}
            )
        except Exception as e:
...
class WebSocketServerThread:
    ...
    def __init__(self, host: str = "localhost", port: int = 8080):
        ...
        self.sync_interval = (
            15  # Sync agent registry every 15 seconds for faster updates
        )
    ...
    def start(self):
        ...
        while wait_time < max_wait:
            try:
                ...
                if result == 0:
                    log_websocket_debug(
                        f"Server is listening on {self.host}:{self.port}"
                    )
                    break
                else:
                    log_websocket_debug(
                        f"Waiting for server to start listening... ({wait_time}s)"
                    )
                    time.sleep(0.5)
                    wait_time += 0.5
            except Exception as e:
        ...
        try:
            logger.info(
                f"Starting WebSocket server periodic sync (every {self.sync_interval}s)"
            )
    ...
    def _schedule_agent_sync(self):
        ...
        if not self.running:
            log_websocket_debug(
                "Not running, returning early from _schedule_agent_sync"
            )
            return
        ...
        if self.sync_in_progress:
            ...
            log_websocket_debug(
                f"Creating timer for next sync in {self.sync_interval} seconds"
            )
            self.sync_timer = threading.Timer(
                self.sync_interval, self._schedule_agent_sync
            )
    ...
    async def _sync_agent_registry(self):
        ...
        try:
            ...
            if agents:
                ...
                if client_count > 0:
                    logger.info(
                        f"Synced agent registry ({len(agents)} agents) to {client_count} clients"
                    )
